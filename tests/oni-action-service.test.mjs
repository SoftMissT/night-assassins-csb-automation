...
describe("oni-action-service", () => {
  it("consome ataque somente nas keys Oni", async () => {
    ...
    assert.equal(parseOniActionState(result.patch["system.props.acoes_oni_dados"]).turn.ataque, 1);
  ...
  it("bloqueia uma segunda acao de ataque sem bonus", async () => {
    const state = { version: 1, turn: { movimento: 0, ataque: 1, especial: 0 }, round: { unica: 0, reacao: 0, lendaria: 0 } };
    const actor = makeActor({ props: { nome_oni: "Akuma", acoes_oni_dados: JSON.stringify(state) } });
    const result = await consumeOniActions(actor, ["ataque"], { update: false });
