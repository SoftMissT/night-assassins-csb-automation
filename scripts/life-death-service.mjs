...
export const LIFE_DEATH_TRIGGER_PROPS = Object.freeze([
    'pdv_slayer_dano_tomado',
    'pdv_slayer_dano_ferida',
    'pdv_slayer_curado',
    'pdv_slayer_extra',
    'status_slayer_exaustao',
...
export function parseLifeDeathState(raw) {
    ...
    try {
        ...
        return {
            ...base,
            ...value,
            dying: Boolean(value?.dying),
            stabilized: Boolean(value?.stabilized),
            dead: Boolean(value?.dead),
            deathMarks: Math.max(0, Math.min(3, Math.trunc(Number(value?.deathMarks) || 0))),
            fallsThisCombat: Math.max(
                0,
                Math.min(4, Math.trunc(Number(value?.fallsThisCombat) || 0))
            ),
            finalDeterminationUsed: Boolean(value?.finalDeterminationUsed),
            bondHelpUsed: Boolean(value?.bondHelpUsed),
            lastPdv: value?.lastPdv === null ? null : Math.max(0, Number(value?.lastPdv) || 0),
            lastDamage: Math.max(0, Number(value?.lastDamage) || 0),
            lastWound: Math.max(0, Number(value?.lastWound) || 0),
        };
    } catch {
...
export function formatLifeDeathSummary(state) {
    ...
    if (state.dying && state.stabilized)
        return `À Beira da Morte · Estabilizado · ${state.deathMarks}/3 Marcas`;
...
function statusPatch(props, { add = [], remove = [], exhaustion = 0, blockReaction = false } = {}) {
    ...
    for (const key of remove) {
        active.delete(key);
        delete status.effects[key];
    }
    for (const key of add) active.add(key);
    if (blockReaction) active.add('sem_reacao');
    status.active = [...active];
    status.exhaustion = Math.min(
        8,
        Math.max(status.exhaustion, Number(props.status_slayer_exaustao) || 0) + exhaustion
    );
    ...
    if (blockReaction) {
        status.effects.sem_reacao = status.effects.sem_reacao ?? {
            damageFormula: '',
            remainingTurns: 1,
            sourceName: 'Vida e Morte',
            saveAttr: '',
...
export function hasLifeDeathTrigger(changes = {}) {
    return LIFE_DEATH_TRIGGER_PROPS.some((key) => changedProp(changes, key) !== undefined);
}

export function isBadCsbNumber(raw) {
    ...
    if (!text) return true;
    if (/\bERROR\b|\[object Object\]/i.test(text)) return true;

    return !/-?\d+(?:[,.]\d+)?/.test(text);
}

export function hasStableSlayerLifeInputs(actor) {
    const props = actor?.system?.props ?? {};
    ...
    if (
        ...
        return false;
    if (
        props.pdv_slayer_dano_ferida !== undefined &&
        typeof props.pdv_slayer_dano_ferida === 'object'
    )
        return false;
    if (props.pdv_slayer_curado !== undefined && typeof props.pdv_slayer_curado === 'object')
        return false;

    return true;
}

export function shouldRunLifeDeath(actor, changes, options) {
    if (options?.naLifeDeath) return false;
    if (!isPrimaryGm()) return false;
    if (!actor?.update) return false;
    if (resolving.has(actor.id)) return false;
    if (!hasLifeDeathTrigger(changes)) return false;
    if (!hasStableSlayerLifeInputs(actor)) return false;
    return true;
}
...
async function markDead(actor, state, reason) {
    ...
    await actor.update(
        { ...lifePatch(state), ...statusPatch(actor.system.props, { remove: ['derrubado'] }) },
        { naCsbAutomation: true, naLifeDeath: true }
    );
    for (const combat of game.combats ?? []) {
        const updates = [...combat.combatants]
            .filter((entry) => entry.actorId === actor.id && !entry.defeated)
            .map((entry) => ({ _id: entry.id, defeated: true }));
    ...
    await ChatMessage.create({
        ...
        content: `<strong>${name} morreu.</strong><br>${safeReason}`,
    });
}
...
async function revive(
    ...
    state,
    pdv,
    { exhaustion = 1, status = null, blockReaction = false, reason = '' } = {}
) {
    const props = actor.system.props;
    state.dying = false;
    state.stabilized = false;
    ...
    await ChatMessage.create({
        speaker: ChatMessage.getSpeaker({ actor }),
        content: `<strong>${name} voltou com ${pdv} PDV.</strong>${safeReason ? `<br>${safeReason}` : ''}`,
    });
}

async function finalDetermination(actor, state, reason) {
    if (state.finalDeterminationUsed)
        return markDead(
            actor,
            state,
            `${reason}<br>Determinação Final já utilizada neste combate.`
        );
    const bondDisabled = state.bondHelpUsed ? ' disabled' : '';
    const decision = await foundry.applications.api.DialogV2.wait({
        window: { title: `Determinação Final ${actor.name}` },
        modal: true,
        rejectClose: false,
        ...
        buttons: [
            {
                action: 'roll',
                label: 'Tentar Determinação',
                callback: (_event, _button, dialog) => {
                    const dcSelect = String(
                        dialog.element.querySelector('[name="dc"]')?.value ?? '18'
                    );
                    const customDc = Number(
                        dialog.element.querySelector('[name="customDc"]')?.value || 0
                    );
                    const dc =
                        dcSelect === 'custom' ? (customDc >= 1 ? customDc : 18) : Number(dcSelect);
                    return {
                        motive: String(
                            dialog.element.querySelector('[name="motive"]')?.value ?? ''
                        ).trim(),
                        dc,
                        bond: Boolean(dialog.element.querySelector('[name="bond"]')?.checked),
                    };
                },
            },
    ...
    if (decision === null || decision === undefined)
        return markDead(actor, state, `${reason}<br>A morte foi aceita.`);
    if (!decision.motive)
        return markDead(actor, state, `${reason}<br>Nenhum Motivo Forte foi declarado.`);
    ...
    await roll.toMessage({
        speaker: ChatMessage.getSpeaker({ actor }),
        flavor: `<strong>Determinação Final</strong> FDV contra CD ${decision.dc}<br>${decision.motive}`,
    });
    ...
    if (natural === 20) {
        ...
        return revive(
            actor,
            state,
            Math.max(1, recovery.total + parseNumber(actor.system.props.vit_display)),
            { exhaustion: 1, status: null, reason: '20 natural na Determinação Final.' }
        );
    }
    if (roll.total >= decision.dc)
        return revive(actor, state, 1, {
            exhaustion: 2,
            status: 'desorientado',
            reason: 'Determinação Final bem-sucedida.',
        });
...
export async function processDeathTest(actor, { force = false } = {}) {
    ...
    await roll.toMessage({
        speaker: ChatMessage.getSpeaker({ actor }),
        flavor: `<strong>Teste de Morte ${actor.name}</strong>`,
    });
    const natural = roll.dice?.[0]?.results?.[0]?.result ?? roll.total;
    if (natural === 1) {
        const recovery = await Roll.create('1d4').evaluate();
        await recovery.toMessage({
            speaker: ChatMessage.getSpeaker({ actor }),
            flavor: 'Corpo resiste recuperação',
        });
        await revive(
            actor,
            state,
            Math.max(1, recovery.total + parseNumber(actor.system.props.vit_display)),
            { exhaustion: 1, status: null, reason: '1 natural no Teste de Morte.' }
        );
    } else if (natural === 20) {
...
async function confirmDirectDeath(actor, state, reason) {
    const decision = await foundry.applications.api.DialogV2.wait({
        window: { title: `Morte sem Teste ${actor.name}` },
        modal: true,
        rejectClose: false,
        ...
        buttons: [
            { action: 'die', label: 'Declarar morte', callback: () => ({ action: 'die', reason }) },
            {
                ...
                label: 'Permitir Determinação Final',
                callback: () => ({ action: 'determination', reason }),
            },
            {
                action: 'fall',
                label: 'Cancelar (queda comum)',
                callback: () => ({ action: 'fall' }),
            },
        ],
    });
...
export async function reconcileActor(actor, options = {}) {
    ...
    try {
        if (!state.dead && parseNumber(props.status_slayer_exaustao) >= 8) {
            ...
            const decision =
                maximum <= 0
                    ? await confirmDirectDeath(
                          actor,
                          state,
                          'Dano de Ferida reduziu o PDV máximo a 0.'
                      )
                    : { action: 'fall' };
            if (decision.action === 'die') {
                ...
                state.dying = true;
                state.stabilized = false;
                state.deathMarks = Math.min(3, Math.max(0, state.fallsThisCombat - 1));
                if (state.fallsThisCombat >= 4)
                    await markDead(actor, state, 'Quarta queda no mesmo combate.');
                else
                    await actor.update(
                        { ...lifePatch(state), ...statusPatch(props, { add: ['derrubado'] }) },
                        { naCsbAutomation: true, naLifeDeath: true }
                    );
            }
        } else if (pdv > 0 && state.dying) {
            await revive(actor, state, pdv, {
                exhaustion: 1,
                status: 'desequilibrado',
                blockReaction: true,
                reason: 'Cura recebida enquanto estava À Beira da Morte.',
            });
        } else if (state.dying && (damage > state.lastDamage || wound > state.lastWound)) {
            state.stabilized = false;
            if (options?.naCritical === true || wound > state.lastWound) {
                await finalDetermination(
                    actor,
                    state,
                    wound > state.lastWound
                        ? 'Dano de Ferida recebido enquanto estava À Beira da Morte.'
                        : 'Dano crítico recebido enquanto estava À Beira da Morte.'
                );
            } else {
                state.deathMarks = Math.min(3, state.deathMarks + 1);
                if (state.deathMarks >= 3)
                    await finalDetermination(
                        actor,
                        state,
                        'Dano recebido enquanto estava À Beira da Morte.'
                    );
                else
                    await actor.update(lifePatch(state), {
                        naCsbAutomation: true,
                        naLifeDeath: true,
                    });
            }
        }
        const previousLastPdv = state.lastPdv;
        const previousLastDamage = state.lastDamage;
        const previousLastWound = state.lastWound;

        state.lastPdv = pdv;
        state.lastDamage = damage;
        state.lastWound = wound;

        const changedSnapshot =
            previousLastPdv !== state.lastPdv ||
            previousLastDamage !== state.lastDamage ||
            previousLastWound !== state.lastWound;
        ...
        const hasCombatState =
            ...
            state.stabilized ||
            state.deathMarks > 0 ||
            state.fallsThisCombat > 0 ||
            damage > 0 ||
            wound > 0;

        if (!state.dead && changedSnapshot && hasCombatState) {
            await actor.update(lifePatch(state), { naCsbAutomation: true, naLifeDeath: true });
        }
    } finally {
...
export async function openLifeDeathManager({ actorUuid } = {}) {
    ...
    const dyingTargets = [...(game.combat?.combatants ?? [])]
    ...
    const targetOptions = dyingTargets
        .map((target) => `<option value="${target.uuid}">${target.name}</option>`)
        .join('');
    const result = await foundry.applications.api.DialogV2.wait({
        window: { title: `Vida e Morte ${actor.name}` },
        modal: true,
        rejectClose: false,
        ...
        buttons: [
            ...(canRoll
                ? [{ action: 'roll', label: 'Rolar Teste agora', callback: () => 'roll' }]
                : []),
            ...(dyingTargets.length
                ? [
                      {
                          action: 'stabilize',
                          label: 'Estabilizar aliado',
                          callback: (_event, _button, dialog) => ({
                              action: 'stabilize',
                              targetUuid: String(
                                  dialog.element.querySelector('[name="target"]')?.value ?? ''
                              ),
                              attr: String(
                                  dialog.element.querySelector('[name="attr"]')?.value ?? 'INT'
                              ),
                          }),
                      },
                  ]
                : []),
            ...(game.user.isGM
                ? [{ action: 'kill', label: 'Declarar morte (sem Teste)', callback: () => 'kill' }]
                : []),
...
async function openDirectDeath(actor, state) {
    ...
    const result = await foundry.applications.api.DialogV2.wait({
        window: { title: `Morte direta ${actor.name}` },
        modal: true,
        rejectClose: false,
        ...
        buttons: [
            {
                action: 'die',
                label: 'Declarar morte',
                default: true,
                callback: (_event, _button, dialog) => ({
                    allowDf: Boolean(dialog.element.querySelector('[name="allowDf"]')?.checked),
                    reason: String(
                        dialog.element.querySelector('[name="reason"]')?.value ?? ''
                    ).trim(),
                }),
            },
...
export async function stabilizeSlayer(helper, target, attr = 'INT', { force = false } = {}) {
    if ((!force && !isPrimaryGm()) || !helper?.update || !target?.update)
    ...
    await roll.toMessage({
        ...
        flavor: `<strong>Estabilizar ${target.name}</strong> ${attribute} CD 12`,
    });
    const natural = roll.dice?.[0]?.results?.[0]?.result;
    ...
    if (targetState.deathMarks >= 3)
        await finalDetermination(target, targetState, 'Falha crítica ao tentar estabilizar.');
...
export function registerLifeDeathEngine() {
    game.socket.on(`module.${MODULE_ID}`, async (message) => {
        ...
        if (
            !requester ||
            !helper?.testUserPermission?.(requester, CONST.DOCUMENT_OWNERSHIP_LEVELS.OWNER)
        )
            return;
        await stabilizeSlayer(helper, target, message.attr, { force: true });
    });
    Hooks.on('updateActor', (actor, changes, options) => {
        if (!shouldRunLifeDeath(actor, changes, options)) return;
    ...
    Hooks.on('combatEnd', (combat) => {
        ...
        for (const combatant of combat.combatants ?? []) {
            ...
            const state = parseLifeDeathState(actor.system.props.vida_morte_slayer_dados);
            state.fallsThisCombat = 0;
            state.finalDeterminationUsed = false;
            state.bondHelpUsed = false;
            state.combatId = '';
