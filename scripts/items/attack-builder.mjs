...
} from './item-technique-normalizers.mjs';
...
function isBreathingForm(item) {
    ...
    return (
        ...
        Boolean(props.forma_id && props.respiracao_nome)
    );
}
...
function attributeValues(actor) {
    ...
    return Object.fromEntries(
        ['vit', 'dex', 'for', 'car', 'fdv', 'int', 'sab'].map((key) => [
            key,
            parseAttributeValue(props[`${key}_display`]),
        ])
    );
}
...
function oniLevel(actor) {
    ...
    return Math.max(
        1,
        Math.trunc(parseNumber(props.nvl_num ?? props.nivel_oni_num ?? props.nvl_pj) || 1)
    );
}
...
function oniUnarmedDefinitions(actor) {
            ...
            damage: [
                {
                    ...
                    label: name,
                    formula: die,
                    fixed: die ? 0 : Number(base),
                    attributeTerms: [{ key: profile.attribute, multiplier: 1, rounding: 'floor' }],
                    types,
                },
            ],
        };
    ...
    return [
        make('martial', 'Ataque Marcial', 'martial', ['concussao']),
        make('claw', 'Garras', 'claw', ['cortante']),
        make('bite', 'Mordida', 'bite', ['perfurante']),
    ].map((definition) => ({
        key: definition.id,
        label: `${definition.name} Nível ${level}`,
        definition,
    }));
}
...
function componentFixed(component, values) {
    ...
    for (const term of component.attributeTerms ?? []) {
        ...
        if (!term.chooseGroup) total += value;
            ...
            grouped.set(
                term.chooseGroup,
                Math.max(grouped.get(term.chooseGroup) ?? Number.NEGATIVE_INFINITY, value)
            );
    }
...
function resolvedFormula(formula, values) {
    return String(formula ?? '').replace(/@([a-z_]+)/giu, (_match, key) =>
...
export function definitionDamageEntries(definition, actor) {
    ...
    return Array.from({ length: attackCount }, (_unused, attackIndex) =>
        (definition?.damage ?? []).map((component) => {
            const secondaryAttack = attackIndex > 0;
            const stripsAttributes =
                secondaryAttack && definition?.metadata?.secondaryDamagePolicy === 'fixo';
            const secondaryComponent = stripsAttributes
...
export function createAttackBuilderModel(actor) {
    ...
    for (const item of itemsOf(actor)) {
                ...
                const normalized = normalizeWeaponTechnique(
                    {
                        ...
                        system: {
                            ...item.system,
                            props: { ...props, arma_perfis_ataque: profiles },
                        },
                    },
                    { profileIndex, ownerKind: ownerKind ?? 'slayer' }
                );
        ...
        if (ownerKind === 'slayer' && isBreathingForm(item)) {
            const normalized = normalizeBreathingTechnique(item, {
                level: breathingLevel,
                ownerKind,
            });
            if (
                !normalized.ok ||
                normalized.definition.metadata.passive ||
                normalized.definition.damage.length === 0
            )
                continue;
            breathing.push({
...
export function buildAttackSelection(
    model,
    { weaponKey = '', breathingKey = '', innateKey = '', manual = false } = {}
) {
    ...
    const definitions = [weapon?.definition, breath?.definition, innate?.definition].filter(
        Boolean
    );
    const entradas = definitions.flatMap((definition) =>
        definitionDamageEntries(definition, model.actor)
    );
    if (weapon && breath) {
        const breathingAction = breath.definition.costs.actions?.[0]?.type;
        if (breathingAction) {
            for (const entry of entradas.filter(
                (candidate) => candidate.sourceId === weapon.definition.id
            ))
                entry.tipoAcao = '';
        }
    }
    const resourceKey = model.ownerKind === 'oni' ? 'pdk' : 'pdr';
    const resourceCost = definitions
        .flatMap((definition) => definition.costs.resources ?? [])
        .filter(
            (cost) =>
                cost.resource === resourceKey ||
                (model.ownerKind === 'oni' && cost.resource === 'pdr')
        )
...
export async function openAttackBuilder(actor) {
    ...
    if (model.weapons.length === 0 && model.breathing.length === 0 && model.innate.length === 0)
        return buildAttackSelection(model, { manual: true });
      ...
      ${model.ownerKind === 'oni' ? `<label>Ataque Demoníaco</label><select name="innateKey">${optionsHtml(model.innate, 'Sem ataque desarmado')}</select>` : `<label>Forma de Respiração</label><select name="breathingKey">${optionsHtml(model.breathing, 'Sem Respiração')}</select>`}
        ...
        buttons: [
            ...
            },
            {
                action: 'manual',
                label: 'Dano Manual',
                callback: () => buildAttackSelection(model, { manual: true }),
            },
