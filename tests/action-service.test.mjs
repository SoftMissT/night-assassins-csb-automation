...
} from '../scripts/action-service.mjs';
...
describe('action-service', () => {
    ...
    it('consome Ataque e impede um segundo uso no mesmo turno', async () => {
        ...
        actor.update = async (patch) => {
            actor.system.props.acoes_slayer_dados = patch['system.props.acoes_slayer_dados'];
        };
    ...
    it('Ação Única mantém teto 1 e bônus amplia as demais', () => {
        assert.deepEqual(
            actionMaximums({
                acoes_slayer_unica_bonus: 9,
                acoes_slayer_ataque_bonus: 2,
                acoes_slayer_reacao_bonus: -1,
            }),
            {
                movimento: 1,
                ataque: 3,
    ...
    it('reset por turno preserva os usos da rodada', async () => {
        const actor = makeSlayer({
            acoes_slayer_dados: JSON.stringify({
                version: 1,
                turn: { movimento: 1, ataque: 1, especial: 1 },
                round: { unica: 1, reacao: 1 },
            }),
        });
        actor.update = async (patch) => {
            actor.system.props.acoes_slayer_dados = patch['system.props.acoes_slayer_dados'];
    ...
    it('enche Fôlego no combate e recupera 1 no início do turno', () => {
        assert.deepEqual(
            slayerFolegoPatch({ fdv_display: 4, folego_slayer_atual: 2 }, { full: true }),
            {
    ...
    it('recupera Fôlego persistente sem ultrapassar o máximo', async () => {
        ...
        assert.deepEqual(await recoverSlayerFolego(actor), {
            changed: true,
            current: 5,
            maximum: 6,
        });
