import { MODULE_ID } from './constants.mjs';

export const SETTINGS = Object.freeze({
    enableSheetAutomation: 'enableSheetAutomation',
    enableDamageRelay: 'enableDamageRelay',
    enableLifeDeathEngine: 'enableLifeDeathEngine',
    criticalFloor: 'stoneCriticalFloor',
    stoneCriticalFloor: 'stoneCriticalFloor',
});

export function registerSettings() {
    game.settings.register(MODULE_ID, SETTINGS.enableSheetAutomation, {
        name: 'NA.Settings.EnableSheetAutomation.Name',
        hint: 'NA.Settings.EnableSheetAutomation.Hint',
        scope: 'world',
        config: true,
        type: Boolean,
        default: true,
        requiresReload: true,
    });

    game.settings.register(MODULE_ID, SETTINGS.enableDamageRelay, {
        name: 'NA.Settings.EnableDamageRelay.Name',
        hint: 'NA.Settings.EnableDamageRelay.Hint',
        scope: 'world',
        config: true,
        type: Boolean,
        default: true,
        requiresReload: true,
    });

    game.settings.register(MODULE_ID, SETTINGS.enableLifeDeathEngine, {
        name: 'NA.Settings.EnableLifeDeathEngine.Name',
        hint: 'NA.Settings.EnableLifeDeathEngine.Hint',
        scope: 'world',
        config: true,
        type: Boolean,
        default: true,
        requiresReload: true,
    });

    game.settings.register(MODULE_ID, SETTINGS.criticalFloor, {
        name: 'NA.Settings.CriticalFloor.Name',
        hint: 'NA.Settings.CriticalFloor.Hint',
        scope: 'world',
        config: true,
        type: Number,
        default: 1,
        range: { min: 1, max: 20, step: 1 },
        requiresReload: false,
    });
}
