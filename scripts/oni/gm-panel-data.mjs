...
export function slayerPanelData(actor) {
  ...
  return Object.freeze({
    ...
    pdr: {
      current: integer(props.pdr_slayer_atual_num ?? (integer(props.pdr_slayer_total_valor) - integer(props.pdr_slayer_gasto_valor))),
...
export function oniPanelData(actor) {
  ...
  return Object.freeze({
    ...
    level,
    rank: regenProfile.available ? (level >= 19 ? "SS" : level >= 16 ? "S" : level >= 12 ? "A" : level >= 7 ? "B" : level >= 3 ? "C" : "D") : null,
...
export function minionPanelData(actor) {
  ...
  return Object.freeze({
    ...
    uuid: actor?.uuid ?? actor?.id ?? "",
    pdv: {
      current: integer(props.oni_minion_pdv_base) - integer(props.oni_minion_pdv_dano) + integer(props.oni_minion_pdv_curado),
      maximum: integer(props.oni_minion_pdv_base),
      dano: integer(props.oni_minion_pdv_dano),
    },
    pdk: {
      current: integer(props.oni_minion_pdk_base) - integer(props.oni_minion_pdk_gasto) + integer(props.oni_minion_pdk_recuperado),
...
export function buildGmPanelData(actors = []) {
  ...
  for (const actor of actors) {
    ...
    else npcs.push(Object.freeze({ kind: "npc", name: cleanName(actor), uuid: actor?.uuid ?? actor?.id ?? "" }));
  }
  return Object.freeze({
