...
export const LIFECYCLE_SCOPES = Object.freeze(["instant", "turn", "round", "combat", "mission", "session", "permanent", "manual"]);
...
const uniqueText = (value) => [...new Set(asArray(value).map((entry) => asText(entry)).filter(Boolean))];
...
function normalizeAttributeTerm(raw = {}) {
  return {
    ...
    rounding: ["floor", "ceil", "round", "none"].includes(raw.rounding) ? raw.rounding : "floor",
...
function normalizeDamageComponent(raw = {}, index = 0) {
  return {
    ...
    split: {
      ...
      rounding: ["floor", "ceil", "round"].includes(raw.split?.rounding) ? raw.split.rounding : "floor",
    },
    criticalPolicy: CRITICAL_POLICIES.includes(raw.criticalPolicy) ? raw.criticalPolicy : "double",
    resistancePolicy: RESISTANCE_POLICIES.includes(raw.resistancePolicy) ? raw.resistancePolicy : "normal",
    woundPolicy: WOUND_POLICIES.includes(raw.woundPolicy) ? raw.woundPolicy : "by-type",
    appliesOn: ["hit", "miss", "save-success", "save-failure", "always"].includes(raw.appliesOn) ? raw.appliesOn : "hit",
  };
}
...
function normalizeStatus(raw = {}, index = 0) {
  ...
  return {
    ...
    maximumStacks: input.maximumStacks == null ? null : Math.max(0, asInteger(input.maximumStacks)),
    ...
    save: input.save ? {
      ...
      timing: ["apply", "turn-start", "turn-end"].includes(input.save.timing) ? input.save.timing : "apply",
...
export function normalizeTechniqueDefinition(raw = {}) {
  return {
    ...
    sourceFamily: SOURCE_FAMILIES.includes(raw.sourceFamily) ? raw.sourceFamily : asText(raw.sourceFamily),
    ...
    targeting: {
      ...
      area: raw.targeting?.area && typeof raw.targeting.area === "object" ? structuredClone(raw.targeting.area) : null,
      disposition: asText(raw.targeting?.disposition, "enemy"),
      distinct: raw.targeting?.distinct === true,
    },
    attack: raw.attack === null || raw.attack === false ? null : {
      ...
      critical: {
        threshold: Math.min(20, Math.max(1, asInteger(raw.attack?.critical?.threshold, 20))),
        disabled: raw.attack?.critical?.disabled === true,
        source: asText(raw.attack?.critical?.source, "definition"),
      },
    },
    defense: raw.defense === null || raw.defense === false ? null : {
...
export function validateTechniqueDefinition(raw = {}) {
  ...
  if (!SOURCE_FAMILIES.includes(definition.sourceFamily)) errors.push(`sourceFamily inválido: ${definition.sourceFamily || "vazio"}.`);
  if (!OWNER_KINDS.includes(definition.ownerKind)) errors.push(`ownerKind inválido: ${definition.ownerKind || "vazio"}.`);
  for (const action of definition.costs.actions) if (!action.type) errors.push(`Custo de ação ${action.id} não possui type.`);
  for (const resource of definition.costs.resources) if (!resource.resource) errors.push(`Custo de recurso ${resource.id} não possui resource.`);
  for (const component of definition.damage) {
    if (!component.formula && component.fixed === 0 && component.attributeTerms.length === 0) errors.push(`Parcela ${component.id} não possui fonte de dano.`);
    if (component.types.length === 0) warnings.push(`Parcela ${component.id} não possui tipo de dano.`);
  }
  for (const status of definition.statuses) {
    if (!status.id) errors.push("Status sem id.");
    if (status.lifecycle.scope !== "instant" && status.lifecycle.scope !== "permanent" && status.lifecycle.scope !== "manual" && status.lifecycle.duration == null) {
      errors.push(`Status ${status.id} exige duration para lifecycle ${status.lifecycle.scope}.`);
    }
  }
  const splitGroups = Object.groupBy(definition.damage.filter((entry) => entry.split.group), (entry) => entry.split.group);
  for (const [group, entries] of Object.entries(splitGroups)) {
    if (entries.reduce((sum, entry) => sum + entry.split.weight, 0) <= 0) errors.push(`Grupo de divisão ${group} possui peso total zero.`);
  }
  if (definition.attack?.count > 1 && !definition.attack.sequential) warnings.push("Ataques múltiplos não sequenciais impedem cancelamento entre tentativas.");
...
export function splitDamageTotal(total, components = []) {
  ...
  return normalized.map((entry, index) => {
    ...
    const value = index === normalized.length - 1
      ? amount - assigned
      : entry.split.rounding === "ceil" ? Math.ceil(raw) : entry.split.rounding === "round" ? Math.round(raw) : Math.floor(raw);
    assigned += value;
