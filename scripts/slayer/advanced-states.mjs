...
export function laminaIgnitionProfile(method) {
  if (method === "sangue") {
    return Object.freeze({ method: "sangue", acao: "especial", custoPdv: "1d4", rastro: 1, estresseInicial: 1 });
  }
  if (method === "atrito") {
    return Object.freeze({ method: "atrito", acao: "especial", custoPdr: 3, rastro: 2, estresseInicial: 1 });
  }
  if (method === "pressao") {
    return Object.freeze({
...
export function laminaRastroEffects(rastro) {
  ...
  if (r === 2) return Object.freeze({ solarDice: "1d6", cauterizacao: "2rodadas", movimento: Object.freeze({ cd: 14, perdeMetros: 3 }) });
  if (r === 3) return Object.freeze({ solarDice: "2d6", cauterizacao: "3rodadas", solarAoBloquear: "1d6" });
  if (r === 4) return Object.freeze({ solarDice: "3d6", cauterizacao: "ateFimCombate", ignoraMetadeRD: true });
...
function mergeState(base, parsed) {
  ...
  state.mundo.atributoLeitura = READING_ATTRIBUTES.includes(String(mundo.atributoLeitura ?? "").toUpperCase())
    ? String(mundo.atributoLeitura).toUpperCase() : "";
  state.mundo.grau = clamp(mundo.grau, 0, 3);
  state.mundo.alvosLidos = mundo.alvosLidos && typeof mundo.alvosLidos === "object" && !Array.isArray(mundo.alvosLidos)
    ? Object.fromEntries(Object.entries(mundo.alvosLidos).map(([uuid, name]) => [String(uuid).slice(0, 200), String(name ?? uuid).slice(0, 120)])) : {};
  ...
  state.altruista.rodadasRestantes = clamp(altruista.rodadasRestantes, 0, ALTRIUSTA_DURACAO_RODADAS);
...
export function formatStatesSummary(state) {
  ...
  if (s.lamina.ativa) parts.push(`Lâmina Carmesim Rastro ${s.lamina.rastro} · Estresse ${s.lamina.estresse}`);
...
export function ativarFoco(state, { alvoUuid, alvoNome = "", grau = 0 } = {}) {
  const s = parseAdvancedStates(state);
  if (!s.mundo.despertado || s.mundo.grau < 1) return { state: s, ok: false, reason: "Mundo Transparente não despertado." };
  ...
  const safe = Math.max(1, s.mundo.usosSeguros || mundoSafeFocusUses(parseNumber(s.mundo.atributoLeitura)));
  const activate = () => {
...
export function ativarLamina(state, { method, cenaOk = false, pdr = 0, level = 0 } = {}) {
  const s = parseAdvancedStates(state);
  if (integer(level) < 10) return { state: s, ok: false, reason: "Requer Exterminador nível 10 ou maior." };
  ...
  if (s.lamina.superaquecida) return { state: s, ok: false, reason: "Lâmina Superaquecida não pode ativar." };
  ...
  if (profile.method === "pressao" && !cenaOk) {
    return { state: s, ok: false, reason: "Pressão exige Marca ativa, PDV abaixo de 25% ou cena extrema." };
  }
...
export function ativarAltruista(state, { pdr = 0 } = {}) {
  const s = parseAdvancedStates(state);
  if (!s.altruista.despertado) return { state: s, ok: false, reason: "Estado Altruísta não despertado." };
  if (s.altruista.ativo) return { state: s, ok: false, reason: "Estado já ativo." };
  if (s.altruista.usadoCombate) return { state: s, ok: false, reason: "Já ativado neste combate." };
  if (s.altruista.travadoDescansoN1) return { state: s, ok: false, reason: "Travado até um Descanso Completo Nível 1." };
  if (integer(pdr) < ALTRIUSTA_CUSTO_PDR) return { state: s, ok: false, reason: "PDR insuficiente." };
...
export function corteSemEgo(state, { pdr = 0 } = {}) {
  const s = parseAdvancedStates(state);
  if (!s.altruista.ativo) return { state: s, ok: false, reason: "Requer Estado Altruísta ativo." };
  if (s.altruista.corteSemEgoUsado) return { state: s, ok: false, reason: "Corte Sem Ego já usado por este personagem." };
  ...
  if (timing === "start") {
    ...
    if (s.lamina.ativa) {
      ...
      if (s.lamina.estresse >= LAMINA_ESTRESSE_MAX) {
        ...
        messages.push("Colapso Carmesim: a arma fica inutilizável até reparo especializado.");
      } else {
        messages.push(`Lâmina Carmesim: Estresse ${s.lamina.estresse}/${LAMINA_ESTRESSE_MAX}`);
      }
      changed = true;
    }
    if (s.altruista.ativo) {
      ...
      if (s.altruista.rodadasRestantes <= 0) {
        ...
        messages.push(`Estado Altruísta: ${s.altruista.rodadasRestantes} rodada(s) restante(s)`);
      }
      changed = true;
    }
  }
...
async function clearCombatStates(combat) {
  ...
  const updates = [...combat.combatants]
    ...
    .map((actor) => saveAdvancedStates(actor, resetPerCombate(actor.system.props[CONTRACT.data])));
 ...
 */
async function commitAdvancedState(actor, nextState, { pdrCost = 0, pdvFormula = "", consumeAction = false, extraPatch = {} } = {}) {
  const props = actor.system?.props ?? {};
  const patch = {};
  if (consumeAction) {
  ...
  if (pdrCost > 0) {
    patch["system.props.pdr_slayer_gasto_valor"] = parseNumber(props.pdr_slayer_gasto_valor) + pdrCost;
  }
  ...
  while (true) {
    ...
    const result = await foundry.applications.api.DialogV2.wait({
      ...
      buttons: [
        ...
        ...(level >= 7 && state.mundo.despertado ? [{ action: "ler-alvo", label: "Ler Alvo", callback: (_event, _button, dialog) => ({ action: "ler-alvo", value: String(dialog.element.querySelector('[name="novoAlvo"]')?.value ?? "") }) }] : []),
        ...(level >= 10 && !state.lamina.ativa && !state.lamina.colapso && !state.lamina.superaquecida ? [{ action: "lamina", label: "Ativar Lâmina", callback: (_event, _button, dialog) => ({ action: "lamina", value: String(dialog.element.querySelector('[name="metodo"]')?.value ?? "sangue") }) }] : []),
        ...(state.lamina.ativa ? [{ action: "apagar", label: "Apagar Lâmina", callback: () => ({ action: "apagar" }) }] : []),
        ...(state.altruista.despertado && !state.altruista.ativo && !state.altruista.usadoCombate && !state.altruista.travadoDescansoN1 ? [{ action: "altruista", label: "Ativar Estado Altruísta (4 PDR)", callback: () => ({ action: "altruista" }) }] : []),
        ...(state.altruista.ativo ? [{ action: "corte-sem-ego", label: "Corte Sem Ego (8 PDR)", callback: () => ({ action: "corte-sem-ego" }) }] : []),
    ...
    if (action === "ler-alvo") {
      ...
      if (!uuid) { ui.notifications.warn("Selecione um alvo do combate."); continue; }
    ...
    if (action === "foco") {
      const uuid = String(result.value ?? "");
      if (!uuid) { ui.notifications.warn("Selecione um Alvo Lido."); continue; }
      const ativado = ativarFoco(state, { alvoUuid: uuid, alvoNome: state.mundo.alvosLidos[uuid] || uuid, grau: state.mundo.grau });
      if (!ativado.ok) { ui.notifications.warn(ativado.reason); continue; }
      if (ativado.extraDc !== undefined) {
        ...
        const commit = await commitAdvancedState(actor, resultado.state, { pdrCost: FOCO_CUSTO_PDR, consumeAction: true });
        if (!commit.ok) { ui.notifications.warn(commit.reason); continue; }
        if (resultado.ativou) ui.notifications.info(`Uso extra de Foco concedido (CD ${ativado.extraDc}).`);
        else ui.notifications.warn("Uso extra falhou: -1 de acerto contra Alvos Lidos até o fim do turno.");
        state = resultado.state;
        continue;
      }
      const commit = await commitAdvancedState(actor, ativado.state, { pdrCost: FOCO_CUSTO_PDR, consumeAction: true });
      if (!commit.ok) { ui.notifications.warn(commit.reason); continue; }
      state = ativado.state;
      continue;
    }
    ...
    if (action === "lamina") {
      ...
      if (!ativada.ok) { ui.notifications.warn(ativada.reason); continue; }
      const profile = laminaIgnitionProfile(method);
      if (profile?.teste) {
        ...
        const danoFormula = resultado.danoSolarInterno ? String(resultado.danoSolarInterno) : "";
        const commit = await commitAdvancedState(actor, resultado.state, { pdrCost: 5, consumeAction: true, pdvFormula: danoFormula });
        if (!commit.ok) { ui.notifications.warn(commit.reason); continue; }
        state = resultado.state;
        continue;
      }
      ...
      const commit = await commitAdvancedState(actor, ativada.state, { pdrCost, pdvFormula, consumeAction: true });
      if (!commit.ok) { ui.notifications.warn(commit.reason); continue; }
      state = ativada.state;
      continue;
    }
    ...
    if (action === "altruista") {
      ...
      if (!ativado.ok) { ui.notifications.warn(ativado.reason); continue; }
      const commit = await commitAdvancedState(actor, ativado.state, { pdrCost: ALTRIUSTA_CUSTO_PDR, consumeAction: true });
      if (!commit.ok) { ui.notifications.warn(commit.reason); continue; }
      state = ativado.state;
      continue;
    }
    ...
    if (action === "corte-sem-ego") {
      ...
      if (!corte.ok) { ui.notifications.warn(corte.reason); continue; }
      const props = actor.system?.props ?? {};
      const { pdrMax } = slayerPdrInfo(props);
      const patch = {
        "system.props.pdr_slayer_gasto_valor": pdrMax + parseNumber(props.pdr_slayer_curado),
      };
