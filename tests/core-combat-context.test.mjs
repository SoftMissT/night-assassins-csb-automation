...
describe("core combat context", () => {
  ...
  it("valida ownerKind, alvo obrigatório e limite", async () => {
    ...
    const missing = validateCombatContext(context, { ownerKind: "slayer", target: { mode: "enemy", maximum: 1 } });
    assert.equal(missing.ok, false);
    assert.match(missing.issues[0], /Nenhum alvo/);
    const wrong = validateCombatContext(context, { ownerKind: "oni", target: { mode: "none" } });
