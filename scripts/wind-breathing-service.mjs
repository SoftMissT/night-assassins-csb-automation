...
export function parseWindBreathingState(raw) {
    if (raw && typeof raw === 'object')
        return { version: 1, scars: 0, battleDamage: { blunt: 0 }, ...raw };
    try {
...
export function registerWindBattleDamage(state, { cutPierce = 0, bleedInfection = 0 } = {}) {
    ...
    return {
        ...
        battleDamage: {
            cutPierce: Math.max(
                0,
                Math.trunc(parseNumber(battleDamage.cutPierce)) +
                    Math.max(0, Math.trunc(Number(cutPierce) || 0))
            ),
            bleedInfection: Math.max(
                0,
                Math.trunc(parseNumber(battleDamage.bleedInfection)) +
                    Math.max(0, Math.trunc(Number(bleedInfection) || 0))
            ),
        },
    };
}
...
export function consolidateWindScars(state) {
    ...
    const newVitBonusFromBleed = Math.min(
        4,
        Math.floor(parseNumber(battleDamage.bleedInfection) / 25)
    );
    return {
        ...state,
...
function resolveFormula(formula, props = {}) {
    return String(formula ?? '').replace(/@(vit|dex|for|car|fdv|int|sab)\b/giu, (_match, key) =>
...
export function buildWindBreathingPlan(formId, level, props = {}, choices = {}) {
    ...
    if (form.passive)
        return {
            ok: false,
            noCost: true,
            reason: 'Tokubetsuna Chi é uma passiva automática e não gasta ação ou PDR.',
        };
    ...
    const emit = ({
        ...
        if (pendingDamage)
            next.pendingDamage = { source: formId, technique: true, ...pendingDamage };
        ...
        return {
            ...
            cost,
            state: next,
            patch: windStatePatch(next, extraPatch),
            ...extra,
        };
    };
    ...
    if (form.id === 'vento_02') {
        ...
        if (requested <= 0)
            return {
                ok: false,
                noCost: true,
                reason: 'Investimento de PDR deve ser maior que zero.',
            };
        if (requested > maxPdr)
            return {
                ok: false,
                noCost: true,
                reason: `Investimento máximo de PDR neste nível: ${maxPdr} (2×DEX).`,
            };
    ...
    if (form.id === 'vento_02_ciclone') {
        ...
        return emit({
            ...
            pendingDamage: {
                ...
                types: selected.damageTypes,
                uses: 1,
                cycloneOpposed: true,
                maxTargets: selected.maxTargets ?? 3,
            },
        });
    }
    if (form.id === 'vento_03') {
        ...
        return emit({
            ...
            pendingDamage: {
                formula: '',
                uses: 1,
                garras: { multiplier: selected.multiplier, addDex: selected.addDex === true },
            },
            extraPatch: {
                'system.props.resp_efeito_flag': `Vento 2: dano da arma ×${selected.multiplier}${selected.addDex ? ' (+DEX antes)' : ''}`,
            },
        });
    }
    if (form.id === 'vento_04') {
    ...
    if (form.id === 'vento_05') {
        ...
        return emit({
            ...
            pendingDamage: {
                formula: '',
                uses: 1,
                disablesHealing: true,
                kekkijutsuSurcharge: level >= 3 ? 2 : 0,
            },
            pendingHit: { count: 1 + (selected.extraAttack ?? 1) },
            extraPatch: {
                'system.props.resp_efeito_flag':
                    'Vento 4: +1 ataque; alvos atingidos sem cura de PDV',
            },
        });
    }
    if (form.id === 'vento_06') {
                ...
                exhaustion > 0
                    ? {
                          'system.props.status_slayer_exaustao': Math.min(
                              8,
                              parseNumber(props.status_slayer_exaustao) + exhaustion
                          ),
                      }
                    : {},
    ...
    if (form.id === 'vento_08') {
        return emit({
            ...
            pendingDamage: {
                ...
                uses: 1,
                ventania: {
                    dcFormula: resolveFormula(selected.saveDc, props),
                    fallDamage: selected.fallDamage,
                },
            },
        });
    }
        ...
        return emit({
            ...
            pendingDamage: {
                ...
                tufao: {
                    ...
                    bleedTurns: 3,
                    healOnBigHit: selected.healOnBigHit ?? 0,
                    healThresholdPercent: selected.healThresholdPercent ?? 10,
                },
            },
            extraPatch: {
                'system.props.status_slayer_exaustao': Math.min(
                    8,
                    parseNumber(props.status_slayer_exaustao) + selected.exhaustionOnUse
                ),
            },
            testDc: selected.testDc,
            bleedSaveDc: resolveFormula(selected.bleedSaveDc, props),
            healOnBigHit: selected.healOnBigHit,
            healThresholdPercent: selected.healThresholdPercent,
        });
    }
