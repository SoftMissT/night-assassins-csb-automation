...
describe("parsing", () => {
  ...
  describe("normalizeAbilityKey", () => {
    it("retorna key direta", () => {
      assert.strictEqual(normalizeAbilityKey("hab_escolhida_marca_destino"), "hab_escolhida_marca_destino");
    });
    it("normaliza por label", () => {
      assert.strictEqual(normalizeAbilityKey("Marca do Destino"), "hab_escolhida_marca_destino");
      assert.strictEqual(normalizeAbilityKey("Tsuyoi — O Inabalável"), "hab_escolhida_tsuyoi");
    });
