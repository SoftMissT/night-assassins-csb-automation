...
test('normaliza status, estado persistido e Exaustão', () => {
    assert.deepEqual(normalizeStatusKeys(['VANTAGEM', 'invalido', 'vantagem', 'atordoamento']), [
        'vantagem',
        'atordoamento',
    ]);
    ...
    assert.deepEqual(
        parseStatusState(
            '<span>{&quot;active&quot;:[&quot;vantagem&quot;],&quot;exhaustion&quot;:0}</span>'
        ).active,
        ['vantagem']
    );
...
test('salva status e Exaustão em uma atualização atômica', async () => {
    ...
    assert.deepEqual(update[0], {
        'system.props.status_slayer_dados':
            '{"version":2,"active":["vulneravel","flanqueado"],"exhaustion":6,"effects":{},"exhaustionMilestones":[]}',
...
test('Sangramento exige dano e quantidade de turnos da fonte', () => {
    assert.deepEqual(
        validateStatusConfiguration(['sangramento'], {
            sangramento: { damageFormula: '', remainingTurns: null },
        }),
        ['Sangramento: informe o dano por turno.', 'Sangramento: informe a quantidade de turnos.']
    );
    assert.deepEqual(
        validateStatusConfiguration(['sangramento'], {
