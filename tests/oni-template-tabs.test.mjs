...
describe("Oni template - abas consolidadas", () => {
  it("tem exatamente 2 abas (Combate e Configs)", () => {
    const tabbed = tpl.system.body.contents.find((c) => c.type === "tabbedPanel");
    assert.ok(tabbed, "Tabbed panel deve existir");
    assert.equal(tabbed.contents.length, 2, "Devem existir 2 abas: Combate, Configs");
  });

  it("aba 0 = Combate com kekkijutsu preservado", () => {
    const tabbed = tpl.system.body.contents.find((c) => c.type === "tabbedPanel");
    ...
    assert.ok(combat.contents.find((c) => c.key === "inventario_oni_kekkijutsus"), "Kekkijutsus devem estar no combate");
  });

  it("aba 1 = Configuracoes preservada", () => {
    const tabbed = tpl.system.body.contents.find((c) => c.type === "tabbedPanel");
    const configs = tabbed.contents[1];
  ...
  it("aba Skills foi removida", () => {
    const tabbed = tpl.system.body.contents.find((c) => c.type === "tabbedPanel");
    const skills = tabbed.contents.find((c) => c.key === "skills_oni_tab");
    assert.ok(!skills, "Aba de Skills nao deve existir");
    const serialized = JSON.stringify(tpl);
    assert.ok(!serialized.includes("kekki_skills_titulo"), "Titulo de skills nao deve existir");
  });
