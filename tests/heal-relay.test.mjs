...
function makeSlayerActor(overrides = {}) {
  return {
    ...
    documentName: "Actor",
    isOwner: true,
    system: { props: { nome_slayer: "Slayer", pdv_slayer_total_valor: 20, pdv_slayer_curado: 0 } },
    update: async () => {},
    ...overrides,
  };
}
...
describe("heal-relay", () => {
  describe("healKeysFor", () => {
    ...
    it("resolve oni_minion_pdv_curado para Oni Minion", () => {
      assert.deepStrictEqual(healKeysFor(makeOniMinionActor()), { heal: "oni_minion_pdv_curado" });
    });
  ...
  describe("applyHealTo", () => {
    it("aplica direto quando o ator que chama é dono do alvo", async () => {
      ...
      const actor = makeSlayerActor({
        isOwner: true,
        update: async (p, o) => { patch = p; options = o; },
      });
    ...
    it("dispara pedido via socket quando quem chama não é dono nem GM", async () => {
      ...
      let handler = null;
      game.socket = {
        emit: (name, message) => { emitted = { name, message }; },
        on: (name, cb) => { handler = cb; },
      };
      registerHealRelay();

      const actor = makeSlayerActor({ isOwner: false, update: async () => { throw new Error("não deveria atualizar direto"); } });
      ...
      handler({ type: "applyHealResult", recipientId: "player_002", requestId: emitted.message.requestId, ok: true, total: 5, appliedHeal: 5, actorName: actor.name });
  ...
  describe("healActor (API pública usada pelo diálogo Dano ou Cura)", () => {
    it("delega para applyHealTo com o mesmo valor calculado como dano", async () => {
      game.user.isGM = true;
      let patch = null;
      const actor = makeSlayerActor({ update: async (p) => { patch = p; } });
  ...
  describe("roteamento do modal Dano ou Cura dentro de rollDamage", () => {
    it("promptHealOrDamage:true + escolha Cura desvia para healActor em vez do dano", async () => {
      ...
      foundry.applications.api.DialogV2.wait = async (config) => {
        ...
        return { nome: "Golpe", pdrGasto: 0, entradas: [{ dado: "1d8", fixo: 0, selAttrs: [], selTiposDano: ["cortante"], tipoAcao: "ataque" }] };
      };
      ...
      const attacker = makeActor({ id: "atk-heal", uuid: "Actor.atk-heal", props: { nome_slayer: "Atacante", pdv_slayer_total_valor: 20 } });
      const target = makeActor({ id: "tgt-heal", uuid: "Actor.tgt-heal", props: { nome_slayer: "Alvo", pdv_slayer_total_conta: 20, pdv_slayer_dano_tomado: 0, pdv_slayer_curado: 0 } });

      let damageApplied = false;
      let healPatch = null;
      target.update = async (patch) => {
        if (patch["system.props.pdv_slayer_dano_tomado"] !== undefined) damageApplied = true;
      ...
      assert.strictEqual(damageApplied, false, "não deveria aplicar dano quando Cura foi escolhida");
    ...
    it("promptHealOrDamage ausente (fallback padrão) nunca abre o modal — comportamento de Dano inalterado", async () => {
      ...
      const attacker = makeActor({ id: "atk-nohealprompt", uuid: "Actor.atk-nohealprompt", props: { nome_slayer: "Atacante", pdv_slayer_total_valor: 20 } });
      const target = makeActor({ id: "tgt-nohealprompt", uuid: "Actor.tgt-nohealprompt", props: { nome_slayer: "Alvo", pdv_slayer_total_conta: 20, pdv_slayer_dano_tomado: 0 } });
      ...
      assert.strictEqual(dialogOpened, false, "sem promptHealOrDamage:true o modal nunca deve abrir (fallback seguro = Dano)");
