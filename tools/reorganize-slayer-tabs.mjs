...
(function walk(node) {
    ...
    if (node.type === 'tabbedPanel' && Array.isArray(node.contents) && node.contents.length >= 8)
        main = node;
...
const findComponentByKey = (node, wanted) => {
    ...
    if (Array.isArray(node)) {
        for (const child of node) {
            const hit = findComponentByKey(child, wanted);
            if (hit) return hit;
        }
        return null;
    }
    if ((node.type === 'itemContainer' || node.type === 'panel') && node.key === wanted)
        return node;
    for (const value of Object.values(node)) {
        const hit = findComponentByKey(value, wanted);
        if (hit) return hit;
    }
    return null;
};
...
const findLabelWithValue = (node, needle) => {
    ...
    if (Array.isArray(node)) {
        for (const child of node) {
            const hit = findLabelWithValue(child, needle);
            if (hit) return hit;
        }
        return null;
    }
    ...
    for (const value of Object.values(node)) {
        const hit = findLabelWithValue(value, needle);
        if (hit) return hit;
    }
    return null;
};
...
const findTextAreaByKey = (node, wanted) => {
    ...
    if (Array.isArray(node)) {
        for (const child of node) {
            const hit = findTextAreaByKey(child, wanted);
            if (hit) return hit;
        }
        return null;
    }
    ...
    for (const value of Object.values(node)) {
        const hit = findTextAreaByKey(value, wanted);
        if (hit) return hit;
    }
    return null;
};
...
const labelModelo =
    findLabelWithValue(notas, 'NOTAS') ??
    findLabelWithValue(perfilAntigo, 'Perfil') ??
    findLabelWithValue(main, 'Rank:');
...
combate.contents.push(
    novoPanel('combate_acumulos_slayer_panel', 'Acúmulos de Respiração', [
        novoLabel(
            '',
            '<span class="na-sheet-text na-sheet-label na-sheet-size-lg na-sheet-role-lime">ACÚMULOS DE RESPIRAÇÃO</span>'
        ),
        ...ESTADOS.map(([nome, key]) =>
            novoLabel(
                '',
                `<span class="na-sheet-text na-sheet-size-md"><strong>${nome}:</strong><br>\${${key}}$</span>`
            )
        ),
    ])
);
...
skills.contents.push(
    novoPanel('alma_lamina_slayer_panel', 'Nichirin / Metais / Alma da Lâmina', [
        novoLabel(
            '',
            '<span class="na-sheet-text na-sheet-label na-sheet-size-lg na-sheet-role-gold">NICHIRIN / METAIS / ALMA DA LÂMINA</span>'
        ),
