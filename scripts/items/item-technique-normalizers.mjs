...
export function normalizeWeaponTechnique(
    item,
    { profileIndex = 0, ownerKind = 'slayer', sourceItemUuid } = {}
) {
    ...
    const definition = normalizeTechniqueDefinition({
        ...
        ownerKind,
        requirements: Array.isArray(props.arma_requisitos_estruturados)
            ? props.arma_requisitos_estruturados
            : [],
        costs: {
            actions: [{ type: 'ataque', amount: 1, timing: 'reserve', refund: 'cancel' }],
            resources: [],
        },
        targeting: {
            mode: 'single',
            count: 1,
            range: rangeMeters(profile.alcance ?? props.arma_alcance),
            disposition: 'enemy',
        },
        attack: {
            attribute: text(
                props.arma_atributo_acerto,
                attackAttributes[0] ?? attributes[0]?.key ?? 'FOR'
            ),
            count: Math.max(
                1,
                number(
                    profile.ataques,
                    propertyMechanics.some((mechanic) =>
                        ['nitoryu', 'ryoto'].includes(mechanic?.id)
                    )
                        ? 2
                        : 1
                )
            ...
            sequential: true,
            critical: {
                threshold: number(profile.critico, number(props.arma_critico, 20)),
                disabled: profile.critico_desabilitado === true,
                source: 'weapon-profile',
            },
        },
        defense: {
            allowed: list(profile.defesas_permitidas ?? ['esquiva', 'bloqueio']),
            onSuccess: text(profile.defesa_sucesso, 'negate'),
        },
...
export function normalizeBreathingTechnique(
    item,
    { level = 1, ownerKind = 'slayer', sourceItemUuid } = {}
) {
    const props = propsOf(item);
    ...
    const passive =
        number(props.forma_passiva) === 1 ||
        (action === '' && /passiva/iu.test(text(props.tipo_manobra)));
    const definition = normalizeTechniqueDefinition({
        ...
        requirements: [
            {
                id: 'breathing-level',
                kind: 'minimum',
                key: 'nvl_respiracao_num',
                operator: '>=',
                value: Math.max(selectedLevel, number(props.nivel_req, 1)),
                reason: 'Nível de Respiração insuficiente.',
            },
        ...
        costs: {
            actions:
                passive || !action
                    ? []
                    : [{ type: action, amount: 1, timing: 'reserve', refund: 'cancel' }],
            resources:
                number(props[`${prefix}_custo`]) > 0
                    ? [
                          {
                              resource: 'pdr',
                              amount: number(props[`${prefix}_custo`]),
                              timing: 'reserve',
                              refund: 'cancel',
                          },
                      ]
        ...
        targeting: props[`${prefix}_alvo_estruturado`] ?? {
            mode: formula || rawStatus ? 'single' : 'self',
            count: 1,
            disposition: formula || rawStatus ? 'enemy' : 'self',
        },
        attack:
            passive || props[`${prefix}_sem_acerto`] === true
                ? null
                : {
                      ...
                      sequential: true,
                      critical: {
                          threshold: number(props[`${prefix}_critico`], 20),
                          source: 'weapon',
                      },
                  },
        defense:
            props[`${prefix}_defesa_estruturada`] ??
            (passive ? null : { allowed: ['esquiva', 'bloqueio'], onSuccess: 'negate' }),
        damage: formula
            ? [
                  {
                      id: `${props.forma_id || 'form'}-damage`,
                      formula,
                      types,
        ...
        statuses: Array.isArray(props[`${prefix}_status_estruturados`])
            ...
            : rawStatus
              ? [{ id: rawStatus, lifecycle: { scope: 'manual' } }]
              : [],
        effects: Array.isArray(props[`${prefix}_efeitos_estruturados`])
            ? props[`${prefix}_efeitos_estruturados`]
            : [],
        lifecycle: props[`${prefix}_ciclo_estruturado`] ?? { scope: 'instant' },
        chat: { summary: text(props[`${prefix}_efeito`], props.descricao) },
        metadata: {
            breathing: text(props.respiracao_nome),
            formId: text(props.forma_id),
            level: selectedLevel,
            passive,
            unresolvedRequirementText: text(props.requisito_texto),
            unresolvedComboText: text(props.combo_texto),
        },
    });
    return validateTechniqueDefinition(definition);
}
