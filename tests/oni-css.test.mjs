...
describe("CSS — skin própria .na-oni-sheet", () => {
  it("declara o token da paleta Oni escopado dentro de .na-oni-sheet, nunca em :root global", () => {
    ...
    for (const token of ["--oni-bg", "--oni-red", "--oni-pdk", "--oni-panel", "--oni-line"]) {
      assert.doesNotMatch(rootBlockMatch[0], new RegExp(token), `${token} vazou para :root global`);
    }
    const oniRootMatch = css.match(/\.na-oni-sheet\s*{[^}]*}/);
    assert.ok(oniRootMatch, ".na-oni-sheet { ... } deve declarar a paleta");
    for (const token of ["--oni-bg", "--oni-red", "--oni-red-dark", "--oni-pdk", "--oni-panel", "--oni-line", "--oni-text"]) {
  ...
  it("possui classes de card Kekkijutsu (badge de PDK, rank, ação, origem, lista)", () => {
    for (const cls of [".na-kekki-card", ".na-kekki-badge-pdk", ".na-kekki-badge-rank", ".na-kekki-badge-action", ".na-kekki-origin", ".na-kekki-list"]) {
      assert.ok(css.includes(`.na-oni-sheet ${cls}`), `classe Kekkijutsu ausente sob .na-oni-sheet: ${cls}`);
    }
  });
  ...
  it("não altera nenhuma regra existente do Slayer (.na-sheet permanece intacto)", () => {
    ...
    assert.ok(slayerSheetBlock, ".na-sheet { ... } (chrome compartilhado) deve continuar existindo");
    ...
    assert.ok(css.includes("--na-gold: #ffd700"), "paleta dourada original do Slayer preservada");
