...
import { allSlayerOriginKeys, originUnlocksAtLevel, validateOriginContract } from "../scripts/slayer/origin-contracts.mjs";
import { allSlayerClassKeys, classRankAtLevel, classUnlocksAtLevel, masterBattleLevelElevenPlan, validateClassContract } from "../scripts/slayer/class-contracts.mjs";
...
test("habilidade de Origem nível 6 não aparece antes do nível 6", () => {
  ...
  assert.deepEqual(originUnlocksAtLevel("origem_samurai", 6).map(({ id }) => id), ["bushido", "iaido"]);
});

test("as cinco Classes possuem contratos C, B, A, S e SS", () => {
  const keys = allSlayerClassKeys();
  assert.equal(keys.length, 5);
  for (const key of keys) assert.deepEqual(validateClassContract(key), { valid: true, ranks: ["C", "B", "A", "S", "SS"] });
});
...
test("Mestre de Batalha nível 11 planeja Corpo de Guerra uma única vez e Aparar por rodada", () => {
  ...
  assert.deepEqual(first.permanentPdv, { formula: "2d6", once: true, stateKey: "slayer_class_mb_corpo_guerra_applied" });
  ...
  const repeated = masterBattleLevelElevenPlan({ nvl_num: 11, classe_escolhida: "classe_mb", slayer_class_mb_corpo_guerra_applied: 1 });
  assert.equal(repeated.permanentPdv, null);
});

test("Mestre de Batalha nível 11 não ativa para outra classe ou nível anterior", () => {
  assert.equal(masterBattleLevelElevenPlan({ nvl_num: 10, classe_escolhida: "classe_mb" }).eligible, false);
  assert.equal(masterBattleLevelElevenPlan({ nvl_num: 11, classe_escolhida: "classe_kakushi" }).eligible, false);
});
