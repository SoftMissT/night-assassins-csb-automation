...
export class ActorTransaction {
    ...
    stage(actor, patch, options = {}) {
        ...
        if (!actor?.uuid || typeof actor.update !== 'function')
            throw new Error('Actor inválido para a transação.');
    ...
    preview() {
        return [...this.#entries.values()].map(({ actor, patch, options }) => ({
            actor,
            patch: { ...patch },
            options: { ...options },
        }));
    ...
    async commit(options = {}) {
        ...
        const results = await Promise.allSettled(
            entries.map(({ actor, patch, options: entryOptions }) =>
                actor.update(patch, {
