...
const BLOCKING_STATUSES = Object.freeze(new Set(["solar", "glicinia", "nichirin", "regeneracao_suprimida"]));
...
export function parseBlockingFlags(props = {}) {
  ...
  for (const status of BLOCKING_STATUSES) {
    if (props[`oni_status_${status}`] !== undefined && props[`oni_status_${status}`] !== false && props[`oni_status_${status}`] !== "false") {
...
export function canActiveRegenerate(level, props = {}, actionState = null) {
  ...
  if (blockedBy.length > 0) return { ok: false, reason: `regeneração bloqueada por: ${blockedBy.join(", ")}.` };
  if (props.oni_regeneracao_usada_turno === true || props.oni_regeneracao_usada_turno === "true") {
  ...
  if (actionState) {
    ...
    for (const action of allowed) {
      const consumed = TURN_ACTIONS.has(action) ? turnConsumed : ROUND_ACTIONS.has(action) ? roundConsumed : turnConsumed;
...
export function canAutomaticRegenerate(level, props = {}) {
  const profile = oniRegenerationProfile(level);
  if (!profile.automaticStartTurnFormula) return { ok: false, reason: "sem regeneração automática neste nível." };
  const { blockedBy } = parseBlockingFlags(props);
  if (blockedBy.length > 0) return { ok: false, reason: `regeneração automática bloqueada por: ${blockedBy.join(", ")}.` };
