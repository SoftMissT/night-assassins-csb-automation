...
import { normalizeTechniqueDefinition, splitDamageTotal, validateTechniqueDefinition } from "../scripts/core/technique-definition.mjs";
import { normalizeBreathingTechnique, normalizeWeaponTechnique } from "../scripts/items/item-technique-normalizers.mjs";

describe("TechniqueDefinition", () => {
  it("normaliza custos, alvo, acerto, dano, status e ciclo", () => {
    const result = validateTechniqueDefinition({
      id: "test:technique", name: "Técnica", sourceFamily: "kekkijutsu", ownerKind: "oni",
      costs: { actions: ["especial"], resources: [{ resource: "pdk", amount: 3, refund: "cancel" }] },
  ...
  it("rejeita status temporal sem duração e parcela vazia", () => {
    const result = validateTechniqueDefinition({
      id: "invalid", sourceFamily: "breathing", ownerKind: "slayer",
      damage: [{}], statuses: [{ id: "sangramento", lifecycle: { scope: "turn" } }],
    });
  ...
  it("divide dano por peso conservando exatamente o total", () => {
    const result = splitDamageTotal(11, [
      { id: "corte", types: ["cortante"], split: { group: "hit", weight: 1, rounding: "floor" } },
      { id: "fogo", types: ["fogo"], split: { group: "hit", weight: 1, rounding: "floor" } },
    ]);
    assert.deepEqual(result.map((entry) => entry.amount), [5, 6]);
    assert.equal(result.reduce((sum, entry) => sum + entry.amount, 0), 11);
  });

  it("preserva políticas distintas por parcela", () => {
    const definition = normalizeTechniqueDefinition({
      id: "split", sourceFamily: "weapon", ownerKind: "slayer",
      damage: [
        { formula: "1d8", types: ["cortante"], resistancePolicy: "normal" },
        { fixed: 3, types: ["ferida"], criticalPolicy: "none", resistancePolicy: "ignore", woundPolicy: "always" },
      ],
    });
...
describe("normalizadores de Item", () => {
  it("transforma perfil de arma em definição executável", () => {
    const result = normalizeWeaponTechnique({ uuid: "Item.weapon", system: { props: {
      arma_nome: "Katana", arma_critico: 19, arma_alcance: "1,5m", arma_propriedades: "Acuidade / Morote",
      arma_perfis_ataque: [{ nome: "Corte", dano_fixo: 4, dano_dados: "1d8", atributos: [{ key: "DEX", multiplicador: 0.5 }], tipos_dano: ["cortante"] }],
    } } });
    assert.equal(result.ok, true);
  ...
  it("transforma nível de Respiração sem fingir duração ausente", () => {
    const result = normalizeBreathingTechnique({ uuid: "Item.breath", system: { props: {
      forma_id: "neve_01", respiracao_nome: "Neve", nome_forma: "Nevasca", tipo_manobra: "Ação Especial", nivel_req: 1,
      nvl2_custo: 3, nvl2_dano: "2d6 + @dex", nvl2_tipos_dano: "congelante", nvl2_status: "Congelar",
    } } }, { level: 2 });
    assert.equal(result.ok, true);
  ...
  it("passiva não reserva ação nem cria acerto", () => {
    const result = normalizeBreathingTechnique({ system: { props: {
      forma_id: "metal_05", respiracao_nome: "Metal", nome_forma: "Martelo", tipo_manobra: "Passiva", forma_passiva: 1,
      nvl1_custo: 0,
    } } });
    assert.equal(result.ok, true);
