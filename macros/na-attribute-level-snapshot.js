...
(async () => {
  ...
  if (!automationModule?.active || typeof automationModule.api?.runAttributeSnapshot !== "function") {
  ...
  const level = input.level ?? input.nvl ?? actor.system?.props?.nvl_pj ?? actor.system?.props?.nvl_num;
  await automationModule.api.runAttributeSnapshot(actor, level);
  if (input.debug === true) {
    console.log(`[TANG-ROU] snapshot de atributos em ${(performance.now() - startedAt).toFixed(2)}ms`);
  }
})();
