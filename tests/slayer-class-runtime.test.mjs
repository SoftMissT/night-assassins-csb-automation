...
describe("class-runtime - Mestre de Batalha", () => {
  ...
  it("ganho permanente de 2d6 PDV so aplica uma vez (idempotente)", () => {
    ...
    const appliedProps = { nvl_num: 11, classe_escolhida: "classe_mb", slayer_class_mb_corpo_guerra_applied: 12 };
...
describe("class-runtime - Usuario de Veneno", () => {
  ...
  it("poisonTick causa dano por stack e decrementa rodadas", () => {
    const result = poisonTick({ slayer_veneno_stacks: 2, slayer_veneno_dano: 4, slayer_veneno_rodadas: 3 });
  ...
  it("poisonTick expira quando rodadas chegam a 0", () => {
    const result = poisonTick({ slayer_veneno_stacks: 1, slayer_veneno_dano: 4, slayer_veneno_rodadas: 1 });
...
describe("class-runtime - Kakushi", () => {
  ...
  it("amparar disponivel no inicio da rodada", () => {
    ...
    assert.equal(kakushiAmpararAvailable({ slayer_class_kakushi_amparar_used_round: 1 }), false);
  });
...
describe("class-runtime - estado e resets", () => {
  it("classStateKey gera chave namespaced", () => {
    assert.equal(classStateKey("classe_mb", "parry_used_round"), "slayer_class_mb_parry_used_round");
    assert.equal(classStateKey("classe_kakushi", "amparar_used_round"), "slayer_class_kakushi_amparar_used_round");
  });
...
describe("class-runtime - event dispatcher", () => {
  ...
  it("physical-melee-damage e aplicavel para rank S", () => {
    const ctx = classEventContext({ classKey: "classe_mb", level: 11, event: "physical-melee-damage" });
