...
async function snapshot(name, sourceDirectory) {
  ...
  const documents = await Promise.all(files.map(async (file) => JSON.parse(await readFile(path.join(sourceDirectory, file), "utf8"))));
  await writeFile(path.join(catalogsDirectory, `${name}.json`), `${JSON.stringify({ format: 1, documents }, null, 2)}\n`);
