...
describe('passivas de Respiração', () => {
    ...
    it('usa o crítico da arma e reduz com Quebra até o limite de FOR', () => {
        ...
        assert.equal(
            effectiveWeaponCritical({ base: 19, state, weaponId: 'katana', strength: 3 }),
            16
        );
        assert.equal(
            effectiveWeaponCritical({ base: 19, state, weaponId: 'machado', strength: 3 }),
            19
        );
    });
    ...
    it('respeita o piso global do crítico sem misturar acúmulos entre armas', () => {
        ...
        assert.equal(
            effectiveWeaponCritical({ base: 19, state, weaponId: 'katana', strength: 8 }),
            11
        );
        assert.equal(
            effectiveWeaponCritical({
    ...
    it('registra Martelo do Julgamento somente após crítico confirmado', () => {
        ...
        assert.deepEqual(state.lastCritical, {
            weaponId: 'katana',
            weaponName: 'Nichirin',
            natural: 18,
            threshold: 18,
...
describe('pipeline confirmado da Pedra', () => {
    ...
    it('recupera somente o dano confirmado no mesmo alvo e turno', () => {
        ...
        assert.equal(
            stoneConfirmedDamageForTarget(state, 'Actor.target', {
                combatId: 'Combat.one',
                round: 3,
                turn: 2,
            }).damage,
        ...
        assert.equal(
            stoneConfirmedDamageForTarget(state, 'Actor.other', {
                combatId: 'Combat.one',
                round: 3,
                turn: 2,
            }),
    ...
    it('limpa Quebra e o último dano quando o combate termina', () => {
        const cleared = clearStonePassiveState({
            stone: {
                breakByWeapon: { katana: 3 },
                lastConfirmedDamageByTarget: { a: { damage: 5 } },
            },
