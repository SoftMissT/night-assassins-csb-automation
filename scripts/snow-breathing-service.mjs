...
export function snowStatePatch(state, overrides = {}) {
    return {
        'system.props.resp_neve_estado': JSON.stringify({ ...EMPTY_STATE(), ...state }),
        ...overrides,
    };
}
...
export function addSnowFreeze(rawState, targetUuid, amount = 1) {
    ...
    return {
        ...
        reachedFive,
        burstFormula:
            reachedFive && state.belowZero?.freezeBurst ? state.belowZero.freezeBurst : null,
    };
}
 ...
 */
export function resolveSnowFreezeGain(
    rawState,
    targetUuid,
    amount = 1,
    { recoveryChoice = '' } = {}
    ...
    const recovery = !canRecover
        ...
        : recoveryChoice === 'pdv'
          ...
          : recoveryChoice === 'pdr'
            ? { resource: 'pdr', amount: 1, pdr: 1 }
            : {
                  resource: 'choice',
                  options: [
                      { resource: 'pdv', amount: 2 },
                      { resource: 'pdr', amount: 1 },
                  ],
              };
...
export function snowEffectiveBreathLevel(baseLevel, rawState) {
    ...
    return Math.min(
        4,
        base + Math.max(0, Math.trunc(parseNumber(state.iceHeart?.breathingLevelBonus)))
    );
}
...
export function resolveSnowRestrictionEscape(flag, rollTotal) {
    if (!flag?.targetUuid)
        return { ok: false, escaped: false, reason: 'Restrição de Congelar ausente.' };
...
export function resolveSnowAvalancheSynergy(
    rawState,
    { targetUuid = '', allyStealthed = false } = {}
) {
    const state = parseSnowBreathingState(rawState);
    const avalanche = state.avalancheTarget;
    const applies = Boolean(
        allyStealthed && avalanche?.turns > 0 && avalanche.uuid && avalanche.uuid === targetUuid
    );
    return {
        applies,
...
export function snowTickPatchWithExhaustion(rawState, currentExhaustion = 0) {
    ...
    const gained = tick.events.reduce(
        (total, event) => total + (event.type === 'exhaustion' ? parseNumber(event.amount) : 0),
        0
    );
    return {
        ...tick,
        patch: {
            ...
            ...(gained > 0
                ? {
                      'system.props.status_slayer_exaustao': Math.min(
                          8,
                          Math.max(0, Math.trunc(parseNumber(currentExhaustion))) + gained
                      ),
                  }
                : {}),
        },
...
export function spendFreezeForRestriction(rawState, targetUuid, car) {
    const state = parseSnowBreathingState(rawState);
    const uuid = String(targetUuid ?? '').trim();
    if (!uuid || snowFreezeCount(state, uuid) < 5)
        return { ok: false, state, reason: 'O alvo precisa ter 5 acúmulos de Congelar.' };
    if (state.restrictedTarget?.uuid === uuid)
        return { ok: false, state, reason: 'O alvo já está restringido pelo Congelar.' };
    ...
    state.restrictedTarget = {
        uuid,
        escapeDc: 8 + Math.trunc(parseNumber(car)),
        breakOnDamage: true,
        escapeAction: 'ataque',
        escapeAttribute: 'FOR',
    };
...
export function breakSnowRestrictionOnDamage(rawState, targetUuid) {
    const state = parseSnowBreathingState(rawState);
    if (state.restrictedTarget?.uuid === targetUuid && state.restrictedTarget.breakOnDamage)
        delete state.restrictedTarget;
    return state;
}

export function grantBlizzardStealth(
    rawState,
    { allyUuid, allyBreathing = '', currentRound } = {}
) {
    const state = parseSnowBreathingState(rawState);
    if (!state.blizzard?.turns) return { ok: false, state, reason: 'Nevasca não está ativa.' };
    if (!allyUuid || allyUuid === state.blizzard.ownerUuid)
        return { ok: false, state, reason: 'Escolha outro aliado.' };
    if (state.blizzard.lastGrantRound === currentRound)
        return { ok: false, state, reason: 'A furtividade já foi concedida nesta rodada.' };
    ...
    return {
        ...
        allyUuid,
        stealth: true,
        pdrRecovery: SNOW_SYNERGIES.includes(allyBreathing) ? 2 : 0,
    };
}
...
export function buildSnowBreathingPlan(formId, level, props = {}, choices = {}) {
    ...
    if (form.cooldown && parseNumber(state.cooldowns?.[formId]) > 0) {
        return {
            ok: false,
            noCost: true,
            reason: `Forma em recarga por ${state.cooldowns[formId]} turno(s).`,
        };
    }
    ...
    if (formId === 'neve_01') {
        ...
        state.pendingTargetEffect = {
            source: formId,
            turns: 2,
            hitPenalty: selected.penalty,
            damagePenalty: selected.penalty,
            vulnerabilities: selected.vulnerabilities ?? [],
        };
    } else if (formId === 'neve_03') {
        state.blizzard = {
            source: formId,
            turns: Math.max(0, car),
            ownerUuid: String(choices.ownerUuid ?? ''),
            lastGrantRound: null,
            grant: 'furtividade',
            synergyBreathings: [...SNOW_SYNERGIES],
            synergyPdrRecovery: 2,
        };
        ...
        state.avalancheTarget = {
            uuid: String(choices.targetUuid ?? ''),
            turns: 2,
            allyStealthBonusDamage: '1d4',
            movementReductionFromRoll: true,
        };
        ...
        state.belowZero = {
            source: formId,
            turns: current?.turns > 0 ? current.turns : 3,
            stacks,
            fdvHitBonus: selected.fdvHit ? fdv * stacks : 0,
            fdvDamageBonus: selected.fdvDamage ? fdv * stacks : 0,
            freezeRecovery: Boolean(selected.freezeRecovery),
            freezeBurst: selected.freezeBurst ?? null,
        };
    } else if (formId === 'neve_07') {
        state.kekkijutsuGuard = {
            source: formId,
            negateEffects: selected.negateEffects,
            damageMultiplier: selected.damageMultiplier,
            protectedUuid: String(choices.protectedUuid ?? choices.ownerUuid ?? ''),
            canProtectAlly: Boolean(selected.protectAlly),
            freezeOnUse: selected.freeze ?? 0,
            opportunityAttack: Boolean(
                selected.protectAlly && SNOW_SYNERGIES.includes(choices.allyBreathing)
            ),
        };
    }
    ...
    Object.assign(
        ...
        snowStatePatch(state, {
            ...
            'system.props.resp_efeito_duracao':
                state.blizzard?.turns ?? state.iceHeart?.turns ?? state.belowZero?.turns ?? 0,
            'system.props.resp_bonus_acerto_temp':
                state.belowZero?.fdvHitBonus ??
                state.iceHeart?.hitBonus ??
                state.nextHit?.bonus ??
                0,
...
export function tickSnowBreathing(raw) {
    ...
    for (const key of [
        ...
        if (state[key]?.turns > 0) {
            state[key].turns -= 1;
            if (state[key].turns <= 0) {
                if (key === 'iceHeart' && state[key].exhaustionOnExpire)
                    events.push({ type: 'exhaustion', amount: state[key].exhaustionOnExpire });
                delete state[key];
            }
        }
    }
...
export function clearSnowBreathingState(rawState) {
    ...
    return snowStatePatch(
        { ...EMPTY_STATE(), freezeByTarget: previous.freezeByTarget },
        {
