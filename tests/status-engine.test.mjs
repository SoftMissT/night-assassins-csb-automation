...
test('Vulnerável e Exaustão 6 dobram somente dano de ataque', () => {
    ...
    assert.deepEqual(resolveIncomingDamage(state, 7, { isAttack: true }), {
        damage: 14,
        vulnerable: true,
    });
    assert.deepEqual(resolveIncomingDamage(state, 7, { isAttack: false }), {
        damage: 7,
        vulnerable: false,
    });
    assert.equal(
        resolveIncomingDamage({ active: [], exhaustion: 6 }, 7, { isAttack: true }).damage,
        14
    );
});
...
test('dano recebido remove Confuso, Distraído e Sonhando', async () => {
    const actor = actorWith({
        ...
        status_slayer_dados: JSON.stringify({
            version: 2,
            active: ['confuso', 'distraido', 'sonhando'],
            exhaustion: 0,
            effects: {},
            exhaustionMilestones: [],
        }),
...
test('Sangramento causa dano no início do turno e expira', async () => {
    ...
    const actor = actorWith({
        ...
        status_slayer_dados: JSON.stringify({
            ...
            exhaustion: 0,
            effects: {
                sangramento: { damageFormula: '1d6', remainingTurns: 1, stacks: 1, tick: 'start' },
            },
            exhaustionMilestones: [],
        }),
    });
...
test('Sangramento mantém e reduz a quantidade de turnos', async () => {
    ...
    const actor = actorWith({
        ...
        status_slayer_dados: JSON.stringify({
            ...
            exhaustion: 0,
            effects: {
                sangramento: { damageFormula: '1d4', remainingTurns: 3, stacks: 1, tick: 'start' },
            },
            exhaustionMilestones: [],
        }),
    });
...
test('Sangramento de Oni não rola no fim do turno', async () => {
    ...
    assert.equal(
        JSON.parse(actor.system.props.status_oni_dados).effects.sangramento.remainingTurns,
        2
    );
});
...
test('danos contínuos de Oni acumulam no mesmo início de turno', async () => {
    ...
    const actor = actorWith({
        ...
        status_oni_dados: JSON.stringify({
            version: 2,
            active: ['sangramento', 'envenenamento'],
            exhaustion: 0,
            effects: {
                sangramento: {
                    damageFormula: '1d4',
...
test('Sangramento incompleto não causa dano nem perde duração', async () => {
    ...
    const actor = actorWith({
        ...
        status_slayer_dados: JSON.stringify({
            ...
            exhaustion: 0,
            effects: {
                sangramento: { damageFormula: '', remainingTurns: null, stacks: 1, tick: 'start' },
            },
            exhaustionMilestones: [],
        }),
    });
...
test('Corroído soma um dado por pilha', async () => {
    ...
    const actor = actorWith({
        ...
        status_slayer_dados: JSON.stringify({
            ...
            exhaustion: 0,
            effects: {
                corroido: { damageFormula: '1d4', remainingTurns: null, stacks: 2, tick: 'start' },
            },
            exhaustionMilestones: [],
        }),
    });
