...
export function metalHammerSynergyAllies(attacker, actors = [], { isAlly = () => true } = {}) {
    return actors.flatMap((actor) => {
        if (!actor?.uuid || actor.uuid === attacker?.uuid || isAlly(actor, attacker) !== true)
            return [];
...
export function spendMetalHammerSynergyPdr(ally, cost = 1) {
    ...
    if (!actorMetalHammerSynergy(ally))
        return { ok: false, reason: 'O aliado não possui Respiração sinérgica.' };
    if (slayerPdrAvailable(props) < amount)
        return { ok: false, reason: 'O aliado não possui PDR suficiente.' };
    return {
        ok: true,
        cost: amount,
        patch: {
            'system.props.pdr_slayer_gasto_valor':
                parseNumber(props.pdr_slayer_gasto_valor) + amount,
        },
    };
}

/** Converte o erro crítico e o acerto da oportunidade em entrada para rollDamage. */
export function buildInabalavelOpportunityDamage(
    raw,
    { enemyNatural = 0, opportunityHit = false } = {}
) {
    ...
    return {
        ...
        entries: [
            {
                tipoAcao: 'reacao',
                dado: resolution.formula,
                fixo: 0,
                attrs: [],
                tiposDano: [resolution.type],
            },
        ],
    };
}
