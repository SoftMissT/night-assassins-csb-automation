...
describe('damage-service', () => {
    it('resolve os danos oficiais de Katana, Double Blade e Manoplas por modo', () => {
        ...
        assert.deepEqual(
            weaponProfileEntries(katanaMorote, attrs).map((entry) => entry.fixo),
            [7]
        );
        assert.deepEqual(
            weaponProfileEntries(doubleBlade, attrs).map((entry) => entry.fixo),
            [5, 5]
        );
        assert.deepEqual(
            weaponProfileEntries({ ...manoplasBase, dano_segundo_golpe: 'fixo' }, attrs).map(
                (entry) => entry.fixo
            ),
            [8, 3]
        );
        assert.deepEqual(
            weaponProfileEntries({ ...manoplasBase, dano_segundo_golpe: 'normal' }, attrs).map(
                (entry) => entry.fixo
            ),
            [8, 8]
        );
    ...
    it('cancela quando dialog retorna null', async () => {
        ...
        await rollDamage({
            actor,
            nome: 'Golpe',
            entradas: [{ dado: '1d8', fixo: 2, attrs: ['for'], tiposDano: ['cortante'] }],
        });
    ...
    it('não quebra quando o diálogo retorna sem entradas', async () => {
        ...
        ui.notifications.warn = (message) => {
            if (message.includes('Adicione ao menos uma entrada')) warned = true;
        };
    ...
    it('aplica PDR e dano em atacante e alvo diferentes', async () => {
        ...
        const target = makeActor({
            id: 'tgt',
            uuid: 'Actor.tgt',
            props: { nome_oni: 'Oni', pdv_oni_dano_tomado: 0 },
        });
    ...
    it('combina PDR e dano quando atacante é alvo', async () => {
        ...
        _dialogReturn = [
            {
                nome: 'Golpe',
                pdrGasto: 2,
                entradas: [{ dado: '1d6', fixo: 0, selAttrs: [], selTiposDano: [], tipoAcao: '' }],
            },
            {
                approved: true,
                normalDamage: 5,
        ...
        const actor = makeActor({
            id: 'self',
            uuid: 'Actor.self',
            props: { nome_oni: 'Oni', pdk_oni_gasto_valor: 1, pdv_oni_dano_tomado: 0 },
        });
    ...
    it('consome a Ação de Ataque do Slayer junto com o gasto de PDR', async () => {
        ...
        const attacker = makeActor({
            id: 'slayer',
            uuid: 'Actor.slayer',
            props: { nome_slayer: 'Slayer', pdv_slayer_total_valor: 20 },
        });
        const target = makeActor({
            id: 'oni',
            uuid: 'Actor.oni',
    ...
    it('rola múltiplas parcelas, usa messageMode v14 e avisa o dano aplicado', async () => {
        ...
        game.settings = {
            get: (namespace, key) =>
                namespace === 'core' && key === 'messageMode' ? 'gm' : undefined,
        };
        _dialogReturn = [
            {
                nome: 'Golpe dividido',
        ...
        const target = makeActor({
            id: 'oni',
            uuid: 'Actor.oni',
            props: { nome_oni: 'Oni', pdv_oni_dano_tomado: 0, pdv_oni_dano_ferida: 0 },
        });
        target.update = async () => {};
        game.user.targets = new Set([{ actor: target }]);
        const notices = [];
    ...
    it('imunidade da Névoa remove o crítico antes da resistência', async () => {
        ...
        const target = makeActor({
            ...
            props: {
                ...
                resp_metal_estado: JSON.stringify({
                    unshakable: { turns: 2, resistances: ['cortante'] },
                }),
            },
        });
        ...
        target.update = async (patch, options) => {
            if (patch['system.props.pdv_slayer_dano_tomado'] !== undefined)
                appliedDamage = patch['system.props.pdv_slayer_dano_tomado'];
    ...
    it('Shi no Kata Hagane no Yō ni Katai N4 (Duro como Aço): anula o dano e oferece contra-ataque real ao defensor', async () => {
        ...
        const attacker = makeActor({
            id: 'atk-steel',
            uuid: 'Actor.atk-steel',
            props: { nome_slayer: 'Atacante', pdv_slayer_total_valor: 20 },
        });
        const target = makeActor({
            id: 'tgt-steel',
            uuid: 'Actor.tgt-steel',
            props: {
                ...
                dex_display: '4',
                resp_metal_estado: JSON.stringify({
                    steelDefense: { negateAttack: true, counterAttack: true, uses: 1 },
                }),
            },
        });
        let appliedDamage = null;
        ...
        target.update = async (patch) => {
            if (patch['system.props.pdv_slayer_dano_tomado'] !== undefined)
                appliedDamage = patch['system.props.pdv_slayer_dano_tomado'];
            if (patch['system.props.resp_metal_estado'] !== undefined)
                steelStateAfter = JSON.parse(patch['system.props.resp_metal_estado']);
        };
        game.user.targets = new Set([{ actor: target }]);
        _rollResult = {
            total: 12,
        ...
        assert.equal(
            steelStateAfter?.steelDefense,
            undefined,
            'steelDefense é consumido em uso único'
        );
    ...
    it('Duro como Aço sem counterAttack (N3): anula o dano mas nunca oferece contra-ataque', async () => {
        ...
        const attacker = makeActor({
            id: 'atk-steel-n3',
            uuid: 'Actor.atk-steel-n3',
            props: { nome_slayer: 'Atacante', pdv_slayer_total_valor: 20 },
        });
        const target = makeActor({
            id: 'tgt-steel-n3',
            uuid: 'Actor.tgt-steel-n3',
            props: {
                nome_slayer: 'Defensor N3',
                pdv_slayer_total_valor: 20,
                pdv_slayer_dano_tomado: 0,
                resp_metal_estado: JSON.stringify({
                    steelDefense: { negateAttack: true, counterAttack: false, uses: 1 },
                }),
            },
        });
        let appliedDamage = null;
    ...
    it('seleciona um perfil de arma e aplica metade do atributo para baixo', async () => {
        ...
        const actor = makeActor({
            props: { dex_display: 5, nome_slayer: 'Slayer', pdv_slayer_total_valor: 20 },
        });
        let chatData;
        ChatMessage.create = async (data) => {
            chatData = data;
        ...
        const item = {
            ...
            system: {
                props: {
                    ...
                    arma_formulas_por_rank: {
                        D: ['5 + metade de FOR ou DEX + FDV + 1d6 / Perfurante'],
                    },
                },
            },
        };
    ...
    it('Água 5 (Chuva Misericordiosa): recupera PDR igual ao Nível de Respiração ao finalizar o alvo (regressão)', async () => {
        ...
        const attacker = makeActor({
            id: 'atk-water5',
            uuid: 'Actor.atk-water5',
            props: {
                nome_slayer: 'Atacante',
                pdv_slayer_total_valor: 20,
                pdr_slayer_gasto_valor: 5,
                resp_agua_estado: JSON.stringify({
                    pendingDamage: {
                        source: 'agua_05',
                        critical: true,
                        uses: 1,
        ...
        attacker.update = async (patch) => {
            if (patch['system.props.pdr_slayer_gasto_valor'] !== undefined)
                attackerPdrGasto = patch['system.props.pdr_slayer_gasto_valor'];
        };
        target.update = async (patch) => {
            if (patch['system.props.pdv_slayer_dano_tomado'] !== undefined)
                target.system.props.pdv_slayer_dano_tomado =
    ...
    it('Água 5: não recupera PDR se o alvo sobreviver ao dano', async () => {
        ...
        assert.equal(
            attackerUpdateCalled,
            false,
            'alvo com PDV restante não deve disparar recuperação de PDR'
        );
    });
});
