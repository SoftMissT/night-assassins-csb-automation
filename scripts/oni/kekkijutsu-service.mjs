...
export function isKekkijutsuItem(item) {
    ...
    return Boolean(
        props.kekki_id || props.kekkijutsu_id || item?.system?.template === 'NAKekkijutsuTpl001'
    );
}
...
export function normalizeKekkijutsu(item) {
    ...
    const damage = [];
    if (props.kekki_dmg_dice)
        damage.push({ dice: props.kekki_dmg_dice, type: props.kekki_dmg_type ?? 'cortante' });
...
export function validateKekkijutsuUse(actor, technique, context = {}) {
    ...
    if (level < techniqueLevel)
        errors.push(`Nível insuficiente: requer ${techniqueLevel}, atual ${level}.`);
    ...
    if (usedThisTurn === true || usedThisTurn === 'true')
        errors.push('Kekkijutsu já usado neste turno.');
    if (technique.limit?.includes('cena') || technique.limit?.includes('combate')) {
        const usedThisScene = props[`kekki_uso_${technique.id}_cena`];
        if (usedThisScene === true || usedThisScene === 'true')
            errors.push('Kekkijutsu já usado nesta cena/combate.');
    }
...
export function buildKekkijutsuAttack(technique, actorAttrs = {}) {
    ...
    return Object.freeze({
        ...
        attributeTerms:
            technique.testType !== 'none' && technique.testType
...
export function buildKekkijutsuPdkPatch(currentGasto, cost) {
    return Object.freeze({
        'system.props.pdk_oni_gasto_valor':
            Math.max(0, integer(currentGasto)) + Math.max(0, integer(cost)),
    });
}
