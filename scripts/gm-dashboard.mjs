...
function fallbackCombatantData(combatant, actor) {
    ...
    const name =
        cleanText(combatant?.name) ||
        cleanText(actor?.name) ||
        (hostile ? 'Oni sem nome' : 'Slayer sem nome');
    return {
        actor,
        kind,
        name,
...
export function hunterData(actor) {
    ...
    const pdvCurrent = firstDefined(props, [
        'pdv_slayer_conta_atual',
        'pdv_slayer_atual_valor_display',
    ]);
    const pdrMax = firstDefined(props, ['pdr_slayer_total_conta', 'pdr_slayer_total_valor']);
    const pdrCurrent = firstDefined(props, [
        'pdr_slayer_conta_atual',
        'pdr_slayer_atual_valor_display',
    ]);
...
export function oniData(actor) {
    ...
    const isMinion =
        ...
        props.oni_minion_pdv_total !== undefined;
    if (isMinion) {
        return {
            actor,
            kind: 'oni',
    ...
    const pdvMax = Math.max(
        0,
        parseNumber(
            firstDefined(props, ['pdv_oni_total_conta', 'pdv_oni_total_valor', 'pdv_total_valor'])
        )
    );
    const pdvCurrent =
        firstDefined(props, [
...
function section(title, icon, entries, kind) {
    ...
    const emptyLabel =
        kind === 'oni'
            ? 'Nenhum Oni identificado pelas keys *_oni*.'
            : 'Nenhum Slayer identificado pelas keys *_slayer*.';
...
export function loadDashboardData(combat = game.combat) {
    const entries = combatantsOf(combat)
        .map((combatant) => {
        ...
        .filter(Boolean);
    const hunters = entries
        .filter((entry) => entry.kind === 'hunter')
        .sort((a, b) => a.name.localeCompare(b.name, 'pt-BR'));
    const onis = entries
        .filter((entry) => entry.kind === 'oni')
        .sort((a, b) => a.name.localeCompare(b.name, 'pt-BR'));
    return { hunters, onis };
}
...
function filterRows(root) {
    const query =
        root.querySelector('.na-gm-mini-search input')?.value.trim().toLocaleLowerCase('pt-BR') ??
        '';
...
function bindDashboard(dialog, element) {
    ...
    root.querySelector('.na-gm-mini-search input')?.addEventListener('input', () =>
        filterRows(root)
    );
...
export async function openGmDashboard() {
    if (!game.user?.isGM)
        return ui.notifications.error('Somente o GM pode abrir o Controle de Combate.');

    if (window.__NAGmDashboard) {
        try {
            await window.__NAGmDashboard.close();
        } catch (_) {
            /* janela anterior já fechada */
        }
    }
