...
describe("oni progression core", () => {
  it("mantém catálogo estruturado completo de 20 níveis", () => {
    const catalog = JSON.parse(fs.readFileSync(new URL("../catalogs/oni-progression.json", import.meta.url), "utf8"));
    assert.equal(catalog.schemaVersion, 1);
    assert.equal(catalog.levels.length, 20);
    assert.deepEqual(catalog.levels.map((entry) => entry.level), Array.from({ length: 20 }, (_, index) => index + 1));
  ...
  it("normaliza nível e resolve todas as patentes canônicas", () => {
    ...
    assert.deepEqual(oniRank(7), { level: 7, title: "Candidato às Doze Kizuki", band: "candidato" });
    assert.deepEqual(oniRank(13), { level: 13, title: "Lua Inferior Um", band: "lua_inferior" });
  ...
  it("escala ataques naturais marciais e de garra ou mordida", () => {
    assert.deepEqual(oniUnarmedProfile(1, "martial"), { style: "martial", attribute: "FOR", formula: "2+FOR", supernatural: false });
  ...
  it("calcula PDV e PDK máximos sem rerrolar parcelas aleatórias", () => {
    const persistedPdvGains = Object.fromEntries(Array.from({ length: 11 }, (_, index) => [`pdv_oni_ganho_nvl${index + 2}`, 4]));
    const level12 = calculateOniResources({ level: 12, originPdv: 20, originPdk: 10, vitality: 4, persistedPdvGains });
    assert.equal(level12.pdvMaximum, 64);
    assert.equal(level12.pdkMaximum, 104);
    assert.equal(level12.randomPdvComplete, true);

    const level20 = calculateOniResources({ level: 20, originPdv: 20, originPdk: 10, vitality: 4, persistedPdvGains });
