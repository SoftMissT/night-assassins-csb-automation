...
function normalizePatterns(rawPatterns) {
  const patterns = { cyclone: emptyPattern(), stigma: emptyPattern(), reflection: emptyPattern() };
...
export function consumeMistPatternBenefit(state, pattern) {
  ...
  normalized.patterns[pattern] = { ...normalized.patterns[pattern], benefitAvailable: false, turnsRemaining: 0 };
  return normalized;
}
...
export function buildMistResistanceSuppressionFlag(sourceActorUuid, turns) {
  return { sourceActorUuid: String(sourceActorUuid ?? ""), turns: Math.max(0, Math.trunc(parseNumber(turns))) };
}
...
export function buildMistBreathingPlan(formId, level, props = {}, choices = {}) {
  ...
  const useCycloneFree = Boolean(choices.useCycloneFree) && state.patterns.cyclone.benefitAvailable;
  ...
  if (formId === "nevoa_01") {
    ...
    state.skySuspended = {
      source: formId, level, damageBonus: { attribute: "SAB", flat: level >= 2 ? level - 1 : 0 },
      consumeOn: "successfulStandardAttack", contactUsesPerAction: 1,
    };
    state.pendingDamage = { source: formId, formula: resolveMistFormula(selected.bonus, props), uses: 1, contactOnce: true, standardAttackOnly: true };
    ...
    state.incomingReduction = {
      source: formId, formula: resolveMistFormula(selected.reduction, props, { level: slayerLevel }),
      rangedOnly: true, level: slayerLevel, sab: level >= 3 ? sab : 0,
    };
    ...
    if (choices.suppressResistance) {
      extraCost += 1;
      const chosenAttribute = choices.suppressAttribute === "for" ? parseNumber(props.for_display) : parseNumber(props.dex_display);
      suppressionTurns = Math.max(0, Math.trunc(chosenAttribute));
    }
    state.pendingDamage = {
      source: formId, formula: selected.damage, uses: 1, replaceWeaponDamage: true,
    ...
    state.mistSea = {
      source: formId, saveDc: selected.saveDc.replace("@sab", String(sab)), targetUuid: String(choices.targetUuid ?? ""),
      halfDamageOnFail: true, normalDamageOnPass: true,
    };
    state.incomingHalfOnFailedSave = { source: formId, saveDc: selected.saveDc.replace("@sab", String(sab)), advantageNextHit: true };
    state.nextHit = { source: formId, advantage: true, targetUuid: String(choices.targetUuid ?? "") };
    ...
    if (choices.dexCheckPassed === false) {
      ...
      state.moonMist = { source: formId, active: true, round: choices.round ?? null, turn: choices.turn ?? null, hitBonusAttribute: "SAB" };
      state.nextHit = { source: formId, count: 1 + extraAttacks, bonus: sab, stopOnMiss: true, criticalBonus: collapseEligible ? fdv : 0 };
    ...
    state.fog = { source: formId, turns, bonus: selected.bonus, enemyUuid: String(choices.targetUuid ?? "") };
  } else if (formId === "nevoa_08") {
    state.obfuscation = {
      source: formId, turns: 5, targetUuid: String(choices.targetUuid ?? ""), allyUuid: String(choices.allyUuid ?? ""),
      hitPenalty: selected.hitPenalty, hitBonus: selected.hitBonus ?? 0,
      exhaustionImmune: true, criticalImmunity: Boolean(selected.criticalImmunity),
    };
