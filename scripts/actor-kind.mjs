...
function templateMarkers(actor) {
    return [
        actor?.id,
        actor?.name,
        actor?.system?.template,
        actor?.system?.templateId,
...
export function actorKind(actor) {
    ...
    const explicitOniMinion =
        ...
        markers.some(
            (value) =>
                value === 'oni_minion_template' ||
                value.includes('oni_minion_template') ||
                ONI_MINION_TEMPLATE_IDS.has(value)
        );
    ...
    const explicitOni =
        ...
        markers.some(
            (value) =>
                value === 'oni_template' ||
                value.includes('oni_template') ||
                ONI_TEMPLATE_IDS.has(value)
        );
    if (explicitOni) return 'oni';

    const explicitSlayer =
        props.nome_slayer !== undefined ||
        props.pdv_slayer_total_conta !== undefined ||
        props.pdv_slayer_total_valor !== undefined ||
        markers.some(
            (value) => value.includes('slayer_template') || SLAYER_TEMPLATE_IDS.has(value)
        );
    if (explicitSlayer) return 'slayer';

    const explicitNpc =
        props.npc_nome !== undefined ||
