...
import {
    buildInterludeProgressPatch,
    executeInterludeActivity,
} from '../scripts/interlude-service.mjs';
...
test('Cabaca acumula sucessos consecutivos e conclui no terceiro', () => {
    ...
    const third = buildInterludeProgressPatch(
        { interludio_cabaca_pequena_sucessos: 2 },
        'cabaca_pequena',
        true
    );
...
test('Falha em Cabaca zera a sequencia', () => {
    const result = buildInterludeProgressPatch(
        { interludio_cabaca_media_sucessos: 2 },
        'cabaca_media',
        false
    );
...
test('Copo de Cha preserva vitorias ao falhar', () => {
    const result = buildInterludeProgressPatch(
        { interludio_copo_cha_vitorias: 2 },
        'copo_cha',
        false
    );
...
test('Cabaca Gigante desbloqueia Concentracao Total Constante', () => {
    const result = buildInterludeProgressPatch(
        { interludio_cabaca_gigante_sucessos: 2 },
        'cabaca_gigante',
        true
    );
    ...
    globalThis.game = {
        dice3d: {
            waitFor3DAnimationByMessageID: async (id) => {
                assert.equal(id, 'roll-1');
                order.push('dice-finished');
            },
        },
    };
    const actor = {
...
test('falha da animação do Dice So Nice não bloqueia o progresso', async () => {
    ...
    globalThis.game = {
        dice3d: {
            waitFor3DAnimationByMessageID: async () => {
                throw new Error('DSN indisponível');
            },
        },
    };
    const actor = {
