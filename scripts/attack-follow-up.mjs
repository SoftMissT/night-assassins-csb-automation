...
export function listChainableBreathForms(actor, { excludeItemUuid = "" } = {}) {
  return [...(actor?.items ?? [])]
    ...
    .map((item) => {
      const props = item.system?.props ?? {};
      return { uuid: item.uuid, label: `${props.respiracao_nome ?? "Respiração"} ${props.nome_forma ?? item.name}` };
    });
}
...
export async function resolveAutoDamage({
  ...
  const actionId = globalThis.foundry?.utils?.randomID?.() ?? globalThis.crypto?.randomUUID?.() ?? String(Date.now());
  for (const attempt of successful) {
    if (weaponItem) {
      ...
      await rollDamage({
        actor, nome: techniqueLabel, entradas, critical: attempt.critical,
        actionId, skipActionConsumption: true, forceAttackDamage,
