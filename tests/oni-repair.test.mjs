...
function legacyOniActor(overrides = {}) {
  ...
  return {
    ...
    items: [{ name: "Kusarigama", type: "item" }, { name: "Sangue das Sombras", type: "item" }],
    getFlag: (mod, key) => flags[`${mod}.${key}`],
    setFlag: async (mod, key, value) => { flags[`${mod}.${key}`] = value; },
...
describe("repairOniActors — migração de Actor Oni legado", () => {
  it("planOniRepair migra Classe legada para Especialização sem apagar o dado antigo", () => {
    ...
    assert.equal(patch["system.props.oni_especializacao_id"], "oni_especializacao_espadachim_profano");
    assert.equal(actor.system.props.classe_escolhida, "classe_oni_espadachim_profano", "valor legado não é apagado");
  ...
  it("repairOniActors preserva nome, imagem, Items, PDV/PDK atual e não cura o Actor", async () => {
    ...
    assert.equal(actor.system.props.pdv_oni_dano_tomado, 35, "dano tomado preservado — não cura ao migrar");
    assert.equal(actor.system.props.oni_especializacao_id, "oni_especializacao_espadachim_profano");
  ...
  it("é idempotente — segunda chamada não gera novo patch", async () => {
    ...
    assert.equal(actor.patches.length, patchesAfterFirst, "nenhum patch adicional na segunda chamada");
  });
  ...
  it("Actor Slayer é ignorado (repair só atua sobre Oni)", async () => {
    const slayer = { name: "Slayer", system: { template: "slayer_template", props: { nome_slayer: "X" } }, update: async () => {}, getFlag: () => undefined, setFlag: async () => {} };
