...
async function doRoll({
    ...
    if (['Bloqueio', 'Esquiva'].includes(test) && naturalD20(roll) === 20)
        await recoverSlayerFolego(actor, 1);
...
export async function rollTest(options) {
    ...
    if (['Bloqueio', 'Esquiva'].includes(test)) {
        ...
        if (mistBonus) {
            statusEffects.modifier += mistBonus;
            statusEffects.reasons.push(`Neblina +${mistBonus}`);
        }
        if (snowBonus) {
            statusEffects.modifier += snowBonus;
            statusEffects.reasons.push(`Coração de Gelo +${snowBonus}`);
        }
    }
    ...
    const bonusRaw = [dialogResult.bonusRaw, derivedTotal ? String(derivedTotal) : '']
        .filter(Boolean)
        .join(' + ');
    ...
    if (metalSteelPatch)
        await actor.update(metalSteelPatch, { naCsbAutomation: true, naBreathing: true });
    ...
    if (defenseSucceeded && test === 'Bloqueio') {
        ...
        if (flameState.block?.intercept) {
            ...
            const interceptorToken =
                actor.getActiveTokens?.()[0] ??
                canvas?.tokens?.controlled?.find((token) => token.actor?.uuid === actor.uuid) ??
                null;
            const distance =
                interceptorToken && protectedToken && canvas?.grid?.measurePath
                    ? Number(
                          canvas.grid.measurePath([interceptorToken.center, protectedToken.center])
                              ?.distance
                      ) || 0
                    : 0;
            if (distance > 3) {
                ui.notifications?.warn?.(
                    `Shi no Kata Sei en no Uneri: o aliado está a ${distance.toFixed(1)}m; o limite é 3m.`
                );
                return result;
            }
            const interception = buildFlameInterception(flameState, {
                interceptorUuid: actor.uuid,
                protectedUuid: protectedActor?.uuid,
            });
            result.interceptAvailable = interception.ok;
            if (interception.ok) {
                await Promise.all([
                    actor.update(interception.patch, { naCsbAutomation: true, naBreathing: true }),
                    protectedActor.setFlag(MODULE_ID, 'flameInterception', interception.flag),
                ]);
                await ChatMessage.create({
                    speaker: ChatMessage.getSpeaker({ actor }),
                    content: `<p><strong>Shi no Kata Sei en no Uneri</strong> — ${protectedActor.name} será interceptado pelo próximo dano recebido.</p>`,
                });
