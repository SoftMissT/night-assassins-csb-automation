...
const MACRO = (id) =>
    `Compendium.night-assassins-csb-automation.night-assassins-macros.Macro.${id}`;
const ROLL = (id, args) =>
    `%{return await (await fromUuid('${MACRO(id)}'))?.execute({actorUuid:entity.uuid${args ? ',' + args : ''}});}%`;
const ATTR_TEST_ROLL = (attr, test, color) =>
    ROLL('NARollMode000001', `test:'${test}',attr:'${attr}',color:'${color}'`);
...
const attributeFields = ATTRS.flatMap(([key, title, color, test, rollColor]) => [
    ...
    label(
        `oni_minion_${key}_display_label`,
        `${title}: \${oni_minion_${key}_display}$`,
        color,
        ATTR_TEST_ROLL(title, test, rollColor)
    ),
]);
...
for (const [key] of ATTRS)
    hidden.push({ name: `${key}_display`, value: `\${oni_minion_${key}_display}$` });
hidden.push(
    {
        name: 'oni_minion_pdv_total',
        value: '${fallback(oni_minion_pdv_base,0)+fallback(oni_minion_nivel,1)+fallback(oni_minion_vit_display,0)}$',
    },
    {
        name: 'oni_minion_pdk_total',
        value: '${fallback(oni_minion_pdk_base,0)+fallback(oni_minion_fdv_display,0)}$',
    },
    {
        name: 'oni_minion_pdv_atual',
        value: '${max(0,oni_minion_pdv_total-fallback(oni_minion_pdv_dano,0)+fallback(oni_minion_pdv_curado,0))}$',
    },
    {
        name: 'oni_minion_pdk_atual',
        value: '${max(0,oni_minion_pdk_total-fallback(oni_minion_pdk_gasto,0)+fallback(oni_minion_pdk_recuperado,0))}$',
    }
);
...
const template = {
    ...
    system: {
        body: {
            contents: [
                ...
                panel('oni_minion_identity', 'Identidade', [
                    textField('oni_minion_nome', 'Nome'),
                    textField('oni_minion_tipo', 'Tipo (Fraco/Comum/Forte)', 'Fraco'),
                    numberField('oni_minion_nivel', 'Nível', 1, 1, 6),
                    textField('oni_minion_pacote', 'Pacote'),
                    textField('oni_minion_ataque', 'Ataque'),
                    textField('oni_minion_traco', 'Traço (exatamente um)'),
                    textField('oni_minion_fraqueza', 'Fraqueza'),
                    textField('oni_minion_comportamento', 'Comportamento'),
                ]),
                ...
                panel('oni_minion_combat', 'Combate', [
                    ...
                    select('acerto_label', 'Escolha como Acerta', [
                        ['acerto_label_escolha', 'Escolha'],
                        ['acerto_label_dex', 'DEX'],
                        ['acerto_label_for', 'FOR'],
                    ]),
                ]),
                panel('oni_minion_resources', 'Recursos', [
                    numberField('oni_minion_pdv_base', 'PDV base', 8, 0),
                    label(
                        'oni_minion_pdv_total_label',
                        'PDV: ${oni_minion_pdv_atual}$ / ${oni_minion_pdv_total}$',
                        '#FF2B4A'
                    ),
                    numberField('oni_minion_pdv_dano', 'Dano tomado', 0, 0),
                    numberField('oni_minion_pdv_curado', 'Curado', 0, 0),
                    numberField('oni_minion_pdk_base', 'PDK base', 2, 0),
                    label(
                        'oni_minion_pdk_total_label',
                        'PDK: ${oni_minion_pdk_atual}$ / ${oni_minion_pdk_total}$',
                        '#B36CFF'
                    ),
                    numberField('oni_minion_pdk_gasto', 'PDK gasto', 0, 0),
                    numberField('oni_minion_pdk_recuperado', 'PDK recuperado', 0, 0),
                ]),
        ...
        hidden,
        attributeBar: {
            oni_minion_pdv_barra: {
                value: '${oni_minion_pdv_atual}$',
                max: '${oni_minion_pdv_total}$',
                editable: false,
            },
            oni_minion_pdk_barra: {
                value: '${oni_minion_pdk_atual}$',
                max: '${oni_minion_pdk_total}$',
                editable: false,
            },
        },
    ...
    prototypeToken: {
        ...
        displayBars: 20,
        actorLink: false,
        bar1: { attribute: 'oni_minion_pdv_barra' },
        bar2: { attribute: 'oni_minion_pdk_barra' },
    },
    items: [],
    effects: [],
    flags: {},
