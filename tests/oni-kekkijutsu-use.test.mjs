...
function fakeActor(props) {
  const patches = [];
  return {
    name: "Oni Teste", uuid: "Actor.oni",
    system: { props },
    update: async (patch) => { patches.push(patch); for (const [k, v] of Object.entries(patch)) props[k.replace(/^system\.props\./, "")] = v; },
    patches,
  };
}
...
describe("useKekkijutsuItem — fluxo USAR na ficha Oni", () => {
  beforeEach(() => {
    globalThis.Roll = { create: (formula) => ({ evaluate: async () => ({ formula, total: 7 }) }) };
  ...
  it("bloqueia por PDK insuficiente sem consumir nada", async () => {
    const item = fakeItem({ kekki_id: "", kekki_nome: "Sangue das Sombras", kekki_pdk_custo: 10, kekki_nivel_desbloqueio: 1, kekki_acao: "especial" });
  ...
  it("bloqueia por nível insuficiente", async () => {
    const item = fakeItem({ kekki_nome: "Técnica Avançada", kekki_pdk_custo: 1, kekki_nivel_desbloqueio: 10, kekki_acao: "especial" });
  ...
  it("uso válido consome PDK, marca o uso do turno e rola dano", async () => {
    const item = fakeItem({
      kekki_nome: "Garras Sombrias", kekki_pdk_custo: 3, kekki_nivel_desbloqueio: 1,
      kekki_acao: "ataque", kekki_dmg_dice: "2d6", kekki_dmg_type: "cortante",
    });
