...
export async function syncCanonicalMacros() {
    ...
    for (const macro of game.macros.contents) {
        const sourceId =
            ...
            macro.flags?.[MODULE_ID]?.sourceId ??
            macro.flags?.core?.sourceId;
    ...
    for (const document of documents) {
        ...
        const source = worldMacroData(
            document,
            existing.folder?.id ?? existing.folder ?? folder.id
        );
        updates.push({
            _id: existing.id,
            name: source.name,
    ...
    return {
        ...
        skipped: documents.length - missing.length - updates.length,
        folderId: folder.id,
    };
}
