...
export function stoneStatePatch(state, overrides = {}) {
    ...
    return {
        ...
        'system.props.resp_pedra_resiliencia_turnos': Math.max(
            0,
            Math.trunc(parseNumber(state.resilience?.turns))
        ),
        ...overrides,
    };
}
...
export function buildStoneBreathingPlan(formId, level, props = {}, choices = {}) {
    ...
    if (form.id === 'pedra_01') {
        next.serpentine = {
            ...
            damageTypes: selected.damageTypes,
            damageComponents: selected.damageComponents,
            saveAttribute: 'VIT',
            saveDc: 10 + strength + Math.floor(Math.max(0, parseNumber(choices.originDamage)) / 10),
            requiresPriorHit: true,
            noHitRoll: true,
        };
        ...
        next.bleeding = {
            amount: selected.bleeding,
            turns: selected.bleedingTurns,
            trigger: 'on-hit',
        };
        ...
        next.reflection = {
            ...
            attribute: ranged ? 'DEX' : 'FOR',
            blockBonus: selected.blockBonus ?? 0,
            blockTurns: selected.blockTurns ?? 0,
            counterAttack: selected.counterAttack === true,
            target: choices.targetUuid ?? '',
            allyTarget: choices.protectedUuid ?? choices.allyUuid ?? '',
        };
        ...
        next.pendingDamage = {
            source: form.id,
            formula: selected.damage,
            formulas: Array(selected.attacks).fill(selected.damage),
            uses: selected.attacks,
            technique: true,
            types: selected.damageTypes,
            recoverPdrOnCritical: selected.recoverPdrOnCritical,
            recoverPdrMaximum: selected.recoverPdrOnCritical,
        };
    ...
    return {
        ...
        actions: form.actions ?? (form.action ? [form.action] : []),
        cost: selected.cost,
        state: next,
...
export function buildStoneMarkReactivation(props = {}) {
    ...
    const current = Math.max(
        0,
        Math.min(maximum, maximum + parseNumber(props.pdr_slayer_curado) - spent)
    );
