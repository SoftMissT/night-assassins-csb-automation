...
import {
  addSnowFreeze, breakSnowRestrictionOnDamage, buildSnowBreathingPlan, clearSnowBreathingState, grantBlizzardStealth,
...
describe("Respiração da Neve", () => {
  ...
  it("restringir de novo exige outra Ação Única: bloqueia enquanto o alvo já está restringido, libera após romper e ainda ter 5+ Congelar", () => {
    ...
    assert.equal(spendFreezeForRestriction(firstRestriction.state, "Actor.Target", 4).ok, false);
  ...
  describe("Combos obrigatórios (seção 38)", () => {
    it("Combo A — Abaixo de Zero N3 + Fluxo de Neve: acerto aplica +1 Congelar e oferece escolha 2 PDV/1 PDR", () => {
      ...
      const gain = resolveSnowFreezeGain(belowZero, "Actor.Oni", 1, { recoveryChoice: "pdv" });
    ...
    it("Combo C — Nevasca + aliado Água/Vento/Cristal: recebe Furtividade + 2 PDR, uma única vez naquela concessão", () => {
      const blizzard = buildSnowBreathingPlan("neve_03", 1, props, { ownerUuid: "Actor.Snow" }).state;
      const granted = grantBlizzardStealth(blizzard, { allyUuid: "Actor.Ally", allyBreathing: "Vento", currentRound: 1 });
      ...
      assert.equal(grantBlizzardStealth(granted.state, { allyUuid: "Actor.Other", allyBreathing: "Água", currentRound: 1 }).ok, false);
      // Respiração sem sinergia: Furtividade concedida, mas sem recuperação de PDR.
      const nextRound = grantBlizzardStealth({ ...blizzard, blizzard: { ...blizzard.blizzard, lastGrantRound: null } }, { allyUuid: "Actor.Other", allyBreathing: "Chamas", currentRound: 2 });
    ...
    it("Combo D — Nevasca + Avalanche acertou o alvo + aliado furtivo pela Nevasca acerta esse alvo: 1d4 único vira dano bônus E redução de deslocamento", () => {
      const avalanche = buildSnowBreathingPlan("neve_05", 3, props, { targetUuid: "Actor.Oni" }).state;
      const synergy = resolveSnowAvalancheSynergy(avalanche, { targetUuid: "Actor.Oni", allyStealthed: true });
      ...
      assert.equal(resolveSnowAvalancheSynergy(avalanche, { targetUuid: "Actor.OutroAlvo", allyStealthed: true }).applies, false);
    });
    ...
    it("Combo F — Canção N4 + Abaixo de Zero N4 + inimigo com 4 Congelar: Canção aplica +1 Congelar chegando a 5 → explosão 6d4 (o source da aplicação não importa)", () => {
      const belowZero = buildSnowBreathingPlan("neve_06", 4, props).state;
      const primed = addSnowFreeze(belowZero, "Actor.Oni", 4).state;
      const songPlan = buildSnowBreathingPlan("neve_07", 4, props, { protectedUuid: "Actor.Snow", allyBreathing: "" });
      ...
      const gain = resolveSnowFreezeGain(primed, "Actor.Oni", songPlan.state.kekkijutsuGuard.freezeOnUse);
      assert.equal(snowFreezeCount(gain.state, "Actor.Oni"), 5);
      assert.equal(gain.burstFormula, "6d4");
    });
  });
  ...
  it("estados simultâneos: tickar Inverno (cooldown) não muta Nevasca/Coração/Abaixo de Zero/Congelar/marca de Avalanche por referência compartilhada", () => {
    const withInverno = buildSnowBreathingPlan("neve_02", 3, props).state;
    const withBlizzard = buildSnowBreathingPlan("neve_03", 2, { ...props, resp_neve_estado: withInverno }, { ownerUuid: "Actor.Snow" }).state;
    const withIceHeart = buildSnowBreathingPlan("neve_04", 2, { ...props, resp_neve_estado: withBlizzard }).state;
    const withBelowZero = buildSnowBreathingPlan("neve_06", 3, { ...props, resp_neve_estado: withIceHeart }).state;
    const withAvalanche = buildSnowBreathingPlan("neve_05", 2, { ...props, resp_neve_estado: withBelowZero }, { targetUuid: "Actor.Marked" }).state;
