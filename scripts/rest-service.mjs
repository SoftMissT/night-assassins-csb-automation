...
const LIGHT_STATUSES = Object.freeze([
  "amedrontado", "desequilibrado", "desorientado", "distraido", "empurrado",
  "surdez_parcial", "cegueira_parcial",
]);
const COMPLETE_STATUSES = Object.freeze([
  ...LIGHT_STATUSES, "sangramento", "hemorragia", "envenenamento", "em_chamas",
  "hipotermia", "confuso", "fadiga_corporal", "fadiga_espiritual", "fadiga_mental",
]);
...
function parseRestData(value) {
  try {
    const text = String(value ?? "").replace(/<[^>]*>/g, "").replaceAll("&quot;", '"').trim();
...
function resourceSnapshot(props = {}) {
  const pdvMaximum = Math.max(0,
    parseNumber(props.pdv_slayer_total_conta) - parseNumber(props.pdv_slayer_dano_ferida) + parseNumber(props.pdv_slayer_extra));
  const pdvCurrent = clamp(
    pdvMaximum + parseNumber(props.pdv_slayer_curado) - parseNumber(props.pdv_slayer_dano_tomado), 0, pdvMaximum);
  const pdrMaximum = Math.max(0,
    parseNumber(props.pdr_slayer_total_conta) + parseNumber(props.metal_slayer_pdr_bonus) + parseNumber(props.pdr_slayer_extra));
  const pdrCurrent = clamp(
    pdrMaximum + parseNumber(props.pdr_slayer_curado) - parseNumber(props.pdr_slayer_gasto_valor), 0, pdrMaximum);
  const breathMaximum = Math.max(0, parseNumber(props.folego_slayer_maximo) || (2 + parseNumber(props.fdv_display)));
...
export function buildRestPatch(props = {}, resolution = {}) {
  ...
  const woundHealing = tier === "deep"
    ? Math.min(parseNumber(props.pdv_slayer_dano_ferida), Math.max(0, Number(resolution.woundHealing) || 0))
    : 0;
  const resourceProps = woundHealing > 0
    ? { ...props, pdv_slayer_dano_ferida: parseNumber(props.pdv_slayer_dano_ferida) - woundHealing }
    : props;
  ...
  const removed = [...new Set(resolution.removeStatuses ?? [])]
    .filter((key) => allowed.has(key) && state.active.includes(key));
  const active = state.active.filter((key) => !removed.includes(key));
  const effects = Object.fromEntries(Object.entries(state.effects).filter(([key]) => active.includes(key)));
  const exhaustion = tier === "field"
    ? state.exhaustion
    : tier === "complete"
      ? Math.max(0, state.exhaustion - 2)
      : resolution.deepExhaustion === "reduce4" ? Math.max(0, state.exhaustion - 4) : 0;
  const nextState = {
    ...state,
    active,
    effects,
  ...
  let pdrRecovered = 0;
  if (tier === "field") {
    pdvRecovered = Math.min(Math.max(0, resources.pdvMaximum - resources.pdvCurrent), Math.max(0, Number(resolution.fieldPdvRoll) || 0));
    pdrRecovered = Math.min(Math.max(0, resources.pdrMaximum - resources.pdrCurrent), Math.floor(resources.pdrMaximum / 2));
    patch["system.props.pdv_slayer_curado"] = parseNumber(props.pdv_slayer_curado) + pdvRecovered;
    patch["system.props.pdr_slayer_curado"] = parseNumber(props.pdr_slayer_curado) + pdrRecovered;
  } else {
...
async function requestRestChoice(actor) {
  return foundry.applications.api.DialogV2.wait({
    ...
    buttons: [
      {
        action: "request", label: "Solicitar ao GM", default: true,
...
function statusChoices(active, tier) {
  ...
  return active.filter((key) => eligible.has(key)).map((key) => `<label style="display:flex;gap:7px;align-items:center">
    <input type="checkbox" name="removeStatus" value="${escapeHtml(key)}">${escapeHtml(STATUS_LABELS.get(key) ?? key)}</label>`).join("");
}
...
async function requestGmApproval(actor, requester, request, tier, fieldRoll) {
  ...
  return foundry.applications.api.DialogV2.wait({
    ...
    ${tier === "deep" ? `<fieldset><legend>Recuperação Profunda</legend>
      ...
      ${state.active.includes("fratura") ? `<label style="display:flex;gap:7px;align-items:center"><input type="checkbox" name="treatFracture">Tratar Fratura agora</label>
      ...
      ${state.active.includes("corrupcao") ? `<label style="display:flex;gap:7px;align-items:center"><input type="checkbox" name="purifyCorruption">Existe ritual ou método narrativo para purificar Corrupção</label>
      ...
      ${parseNumber(props.pdv_slayer_dano_ferida) > 0 ? `<div class="form-group"><label>PDV máximo devolvido por tratamento importante</label><div class="form-fields"><input type="number" name="woundHealing" min="0" max="${parseNumber(props.pdv_slayer_dano_ferida)}" value="0"></div></div>
    ...
    buttons: [
      ...
      {
        action: "approve", label: "Autorizar e aplicar", default: true,
        callback: (_event, button) => {
          const form = new FormData(button.form);
          return {
            approved: form.get("progressConfirmed") === "on",
            error: form.get("progressConfirmed") === "on" ? "" : "Confirme a regra antiabuso para autorizar.",
...
async function rollDeepTreatments(actor, approval) {
  ...
  if (approval.treatFracture) {
    const modifier = approval.fractureAttr === "SAB" ? approval.healerSab : parseNumber(actor.system?.props?.vit_display);
    const roll = await new Roll(`1d20 + ${modifier}`).evaluate();
    const natural = Number(roll.dice?.[0]?.results?.find?.((result) => result.active !== false)?.result) || 0;
    await roll.toMessage({ speaker: ChatMessage.getSpeaker({ actor }), flavor: "Recuperação Profunda — tratamento de Fratura contra CD 14" });
    if (Number(roll.total) >= 14 || natural === 20) removeStatuses.push("fratura");
    notes.push(natural === 1 ? "Fratura: falha crítica; o GM pode aplicar Fadiga Corporal." : Number(roll.total) >= 14 ? "Fratura tratada." : "Fratura permanece.");
  }
  if (approval.purifyCorruption) {
    const modifier = parseNumber(actor.system?.props?.fdv_display);
    const roll = await new Roll(`1d20 + ${modifier}`).evaluate();
    await roll.toMessage({ speaker: ChatMessage.getSpeaker({ actor }), flavor: `Recuperação Profunda — purificação de Corrupção contra CD ${approval.corruptionDc}` });
    if (Number(roll.total) >= approval.corruptionDc) removeStatuses.push("corrupcao");
    notes.push(Number(roll.total) >= approval.corruptionDc ? "Corrupção purificada." : "Corrupção permanece.");
  }
...
async function applyApprovedRest(actor, requester, request) {
  ...
  if (roll) await roll.toMessage({ speaker: ChatMessage.getSpeaker({ actor }), flavor: `${RESTS[tier].label} — 1d4 × VIT (${vitality})` });
  ...
  const treatments = tier === "deep" ? await rollDeepTreatments(actor, approval) : { removeStatuses: approval.removeStatuses, notes: [] };
  const record = {
    version: 1,
...
function emitResult(message, result) {
  game.socket.emit(SOCKET_NAME, { type: RESPONSE_TYPE, recipientId: message.requesterId, requestId: message.requestId, ...result });
}
...
async function handleRestRequest(message) {
  ...
  if (!requester?.active || !actor) return emitResult(message, { ok: false, error: "Pedido de descanso inválido." });
  try {
...
export async function openRestManager({ actorUuid } = {}) {
  ...
  if (!actor.isOwner && !game.user.isGM) return ui.notifications.error("Você não pode solicitar descanso para este personagem.");
  const request = await requestRestChoice(actor);
  if (!request) return null;
  const tier = resolveRestTier(request.requested, request.completedHours);
  if (!tier) return ui.notifications.warn("O descanso foi interrompido antes do benefício mínimo.");
  const gm = primaryActiveGm();
  if (!gm) return ui.notifications.error("Nenhum GM ativo para autorizar o descanso.");
  if (game.user.isGM && gm.id === game.user.id) return applyApprovedRest(actor, game.user, request);
