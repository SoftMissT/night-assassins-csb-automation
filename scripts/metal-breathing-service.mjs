...
function metalSummary(state) {
  const active = [];
  if (state.metalized?.turns > 0) active.push(`Metalizado: Bloqueio +${state.metalized.blockBonus} (${state.metalized.turns}t)`);
  if (state.unshakable?.turns > 0) active.push(`Inabalável: resistência Cortante/Perfurante (${state.unshakable.turns}t)`);
  if (state.chainReaction?.turns > 0) active.push(`Reação em Cadeia: +${state.chainReaction.damageBonus} (${state.chainReaction.turns}t)`);
  if (state.battleForged?.turns > 0) active.push(`Forjado: ${state.battleForged.stageName || "sem acerto"} (${state.battleForged.turns}t)`);
...
export function metalStatePatch(state, overrides = {}) {
  ...
  return {
    ...
    "system.props.resp_metal_bloqueio_bonus": Math.max(0, Math.trunc(parseNumber(normalized.metalized?.blockBonus))),
    "system.props.resp_metal_for_temp": Math.max(0, Math.trunc(parseNumber(normalized.battleForged?.forBonus))),
    "system.props.resp_metal_fdv_temp": Math.max(0, Math.trunc(parseNumber(normalized.battleForged?.fdvBonus))),
    ...overrides,
  };
}
...
export function buildMetalBreathingPlan(formId, level, props = {}, choices = {}) {
  ...
  if (form.id === "metal_01") {
    if (state.metalized?.turns > 0) return { ok: false, noCost: true, reason: "Metalizado já está ativo." };
    next.metalized = {
      turns: selected.turns,
      blockBonus: selected.blockScale === "vit" ? vit : Math.ceil(vit / 2),
    };
  } else if (form.id === "metal_02") {
    if (state.unshakable?.turns > 0) return { ok: false, noCost: true, reason: "Inabalável já está ativo." };
    next.unshakable = {
      turns: selected.turns,
      resistances: ["cortante", "perfurante"],
      multiplier: 0.5,
      enemyCriticalFailureOpportunity: { damage: `${level}d4`, type: "concussao", requiresHit: true },
    };
  } else if (form.id === "metal_03") {
    if (state.chainReaction?.turns > 0) return { ok: false, noCost: true, reason: "Reação em Cadeia já está ativa." };
    next.chainReaction = {
      turns: fdv,
  ...
  return { ok: true, form, selected, action: form.action, actions: [form.action], cost: selected.cost, state: next, patch: metalStatePatch(next) };
}
...
export function buildMetalHammerFollowUp({ breathingLevel = 0, originalDamage = 0, synergyBreathing = "", allySpendsPdr = false } = {}) {
  const level = Math.max(0, Math.trunc(parseNumber(breathingLevel)));
  if (level < METAL_HAMMER_PASSIVE.minimumLevel) return { ok: false, reason: "Martelo do Julgamento requer Nível de Respiração 1." };
  const synergy = METAL_HAMMER_PASSIVE.synergyBreathings.includes(String(synergyBreathing).trim().toLowerCase()) && allySpendsPdr;
  const multiplier = synergy ? METAL_HAMMER_PASSIVE.synergyDamageMultiplier : METAL_HAMMER_PASSIVE.damageMultiplier;
  return {
    ok: true,
...
export function consumeMetalHammerPending(passiveState = {}, options = {}) {
  ...
  if (metal.hammerPending !== true) return { ok: false, consumed: false, state, reason: "Martelo do Julgamento não está liberado." };
...
export function resolveMetalCriticalFailureOpportunity(raw, { enemyCriticalFailure = false, opportunityHit = false } = {}) {
  const state = parseMetalBreathingState(raw);
  const opportunity = state.unshakable?.turns > 0 ? state.unshakable.enemyCriticalFailureOpportunity : null;
  if (!opportunity || enemyCriticalFailure !== true) return { eligible: false, applyDamage: false, formula: "", type: "" };
  return {
    eligible: true,
...
export function registerMetalChainApplication(raw, actionId) {
  const state = parseMetalBreathingState(raw);
  const id = String(actionId ?? "").trim();
  if (!canApplyMetalChain(state, id)) return { changed: false, state, patch: metalStatePatch(state) };
  const previous = Array.isArray(state.chainReaction.appliedActionIds) ? state.chainReaction.appliedActionIds : [];
  state.chainReaction = { ...state.chainReaction, appliedActionIds: [...previous, id].slice(-20) };
...
export function resolveMetalChainReroll(raw, { originalTotal = 0, rerollHit = false } = {}) {
  ...
  return { allowed, exhaustion: allowed && !rerollHit ? state.chainReaction.exhaustionOnFailedReroll : 0 };
}
