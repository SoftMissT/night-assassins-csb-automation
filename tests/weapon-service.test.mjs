...
describe('weapon-service', () => {
    ...
    it('resolve os atributos de acerto pelas propriedades canônicas', () => {
        ...
        assert.deepEqual(weaponAttackAttributes({ arma_propriedades: '** Acuidade / Morote' }), [
            'FOR',
            'DEX',
        ]);
    ...
    it('não inventa dano de atributo para Acuidade ou Morote', () => {
        ...
        const [unproficient] = weaponProfilesForActor(itemProps, {
            nvl_num: 1,
            armas_proficientes: 'Rapieira',
        });
    ...
    it('filtra perfis pelo modo persistido no Item', () => {
        ...
        assert.deepEqual(
            weaponUsageModes(props).map((entry) => entry.key),
            ['nitoryu', 'morote']
        );
    ...
    it('aceita perfis e fórmulas estruturados persistidos como JSON textual', () => {
        const [profile] = weaponProfilesForActor(
            {
                arma_categoria: 'especial',
                arma_perfis_ataque: JSON.stringify([
                    {
                        nome: 'Corte',
                        dano_fixo: 4,
                        atributos: JSON.stringify([{ key: 'FOR', multiplicador: 1 }]),
                        tipos_dano: ['cortante'],
                    },
                ]),
    ...
    it('normaliza rótulos de dano para as chaves do relay', () => {
        assert.deepEqual(weaponDamageTypeKeys('Cortante ou Concussivo, à escolha no acerto'), [
            'cortante',
            'concussao',
        ]);
    });
    ...
    it('bloqueia um perfil que exige dois disparos sem munição suficiente', () => {
        const state = weaponAmmoState(
            { arma_municao_capacidade: 2, arma_municao_atual: 1 },
            { ataques: 2 }
        );
        assert.deepEqual(state, { capacity: 2, current: 1, shots: 2, required: true });
        assert.deepEqual(weaponAmmoPatch({ arma_municao_capacidade: 2 }, -1), {
            'system.props.arma_municao_atual': 0,
        });
    });
});
