...
export async function rollOniPdvGain(actor, { level, onlyMissing = true, showDice = true } = {}) {
    ...
    const normalized = normalizeOniLevel(
        level ?? props.nvl_oni ?? props.nvl_num ?? String(props.nvl_pj ?? '').replace(/^nvl_/, '')
    );
    ...
    console.warn(
        `[NA-ONI-PDV] PLAN ${plan.length} gains:`,
        plan.map((e) => `nvl${e.level}(${e.dice})`)
    );
    const results = [];
    const patch = {};
    for (const entry of plan) {
        ...
        try {
            ...
            console.warn(
                `[NA-ONI-PDV] ROLL RESULT level=${entry.level} dice=${entry.dice} result=${rollTotal}`
            );
            ...
            if (showDice && typeof roll.toMessage === 'function') {
                ...
                try {
                    ...
                    console.warn(
                        `[NA-ONI-PDV] DICE3D fallback displayed=true level=${entry.level}`
                    );
                } catch (diceError) {
            ...
            globalThis.ui?.notifications?.error?.(
                `Falha ao rolar ganho do nível ${entry.level}: ${error.message}`
            );
            continue;
        }
    ...
    const remaining = missingOniPdvGains(normalized, {
        ...props,
...
export async function ensureOniProgression(actor, { level, showDice = true } = {}) {
    ...
    console.warn(
        `[NA-ONI-PDV] LEVEL CHANGE actor=${actor.name} level=${normalized} missing=${missing.length}`
    );
    if (!missing.length) {
...
export function calculateOniResources({
    level,
    originPdv = 0,
    originPdk = 0,
    vitality = 0,
    ...
    return Object.freeze({
        ...
        breakdown: Object.freeze({
            originPdv: Math.max(0, integer(originPdv)),
            randomPdv: random.total,
            fixedPdv,
            originPdk: Math.max(0, integer(originPdk)),
            pdkGained,
        }),
    });
}
