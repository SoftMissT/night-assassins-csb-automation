...
describe("Respiração do Metal — missão: estados simultâneos, legado e combos", () => {
  ...
  it("§35 · cinco estados ativos: consumir um não apaga os outros", () => {
    let state = {};
    for (const [id, level] of [["metal_01", 2], ["metal_02", 1], ["metal_03", 3], ["metal_04", 2], ["metal_06", 1]]) {
      state = buildMetalBreathingPlan(id, level, { ...VIT, resp_metal_estado: JSON.stringify(state) }).state;
    }
    assert.ok(state.metalized && state.unshakable && state.chainReaction && state.steelDefense && state.battleForged);
  ...
  it("§36 · estado legado sem campos novos normaliza sem quebrar", () => {
    ...
    const plan = buildMetalBreathingPlan("metal_04", 3, { ...VIT, resp_metal_estado: JSON.stringify(legacy) });
    assert.equal(plan.ok, true);
    assert.equal(plan.state.metalized.blockBonus, 3);
    assert.ok(plan.state.chainReaction.damageBonus === 6 || plan.state.chainReaction === undefined === false);
  });
  ...
  it("Combo A · Metalizado + Duro como Aço N2 compõem Bloqueio + vantagem", () => {
    ...
    const steel = buildMetalBreathingPlan("metal_04", 2, { ...VIT, resp_metal_estado: JSON.stringify(state) });
  ...
  it("Combo C · Reação em Cadeia + Forjado coexistem (dano da 3ª + FOR/FDV temporários)", () => {
    ...
    state = buildMetalBreathingPlan("metal_06", 1, { ...VIT, resp_metal_estado: JSON.stringify(state) }).state;
...
describe("Respiração do Metal — Magnetismo permanente (decisão do Operador)", () => {
  it("é elegível SEM Forjado ativo — passiva permanente", () => {
    const state = buildMetalBreathingPlan("metal_06", 1, { vit_display: 7, for_display: 4 }, { targetUuid: "Actor.inimigo" }).state;
    ...
    assert.deepEqual(check, { active: true, valid: false, requiredTargetUuid: "Actor.inimigo" });
  ...
  it("não é elegível com VIT/FOR < 6 — nenhum alvo é gravado", () => {
    const state = buildMetalBreathingPlan("metal_06", 1, { vit_display: 5, for_display: 5 }, { targetUuid: "Actor.inimigo" }).state;
  ...
  it("estado legado (battleForged.magnetismEligible) continua validando", () => {
    const legacy = {
      version: 1,
      battleForged: { turns: 4, magnetismEligible: true, magnetismTargetUuid: "Actor.legado" },
    };
    assert.deepEqual(validateMetalMagnetismTarget(legacy, ["Actor.outro"]), { active: true, valid: false, requiredTargetUuid: "Actor.legado" });
  ...
  it("fim do combate limpa o alvo obrigatório do Magnetismo", async () => {
    ...
    const state = buildMetalBreathingPlan("metal_06", 1, { vit_display: 7 }, { targetUuid: "Actor.inimigo" }).state;
    const cleared = parseMetalBreathingState(JSON.stringify({})).version ? clearMetalBreathingState(JSON.stringify(state)) : null;
    assert.ok(cleared);
