...
import {
  normalizeKekkijutsu, validateKekkijutsuUse, buildKekkijutsuAttack,
  buildKekkijutsuUsePatch, buildKekkijutsuPdkPatch,
...
export async function useKekkijutsuItem({ item, actor } = {}) {
  ...
  const validation = validateKekkijutsuUse(actor, technique, {
    level: props.nvl_num, currentPdk: props.pdk_oni_atual_num,
  });
  if (!validation.ok) {
    globalThis.ui?.notifications?.warn?.(`Kekkijutsu bloqueado: ${validation.errors.join(" ")}`);
  ...
  if (attack.testFormula) {
    try {
      ...
      rolls.push(testRoll);
    } catch { /* fórmula de teste inválida/ausente — segue sem rolagem de teste */ }
  }
  for (const component of attack.damage) {
    try {
      const damageRoll = await globalThis.Roll.create(component.formula).evaluate();
      rolls.push(damageRoll);
    } catch { /* fórmula de dano inválida/ausente — ignora componente */ }
  }
