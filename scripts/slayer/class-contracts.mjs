...
export function classUnlocksAtLevel(key, rawLevel) {
  ...
  return RANK_ORDER
    .filter((rank) => classCatalog.rankLevels[rank] <= level)
    .map((rank) => ({ rank, level: classCatalog.rankLevels[rank], ...structuredClone(definition.ranks[rank]) }));
}
...
export function masterBattleLevelElevenPlan(props = {}) {
  ...
  return {
    eligible: Math.trunc(Number(props.nvl_num ?? props.level)) >= 11
      && String(props.classe_escolhida ?? props.classKey) === "classe_mb",
    permanentPdv: alreadyApplied ? null : {
