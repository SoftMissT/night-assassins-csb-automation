...
const PROPS_BASE = { dex_display: 4, fdv_display: 3, vit_display: 5, for_display: 3, status_slayer_exaustao: 0 };
...
describe("Respiração do Vento — Sangue Especial (passiva)", () => {
  ...
  it("fim de combate limpa apenas o pool da batalha — cicatrizes persistem", () => {
    const state = consolidateWindScars(registerWindBattleDamage(parseWindBreathingState({}), { cutPierce: 30 }));
    const cleared = parseWindBreathingState(JSON.parse(JSON.stringify(clearWindBreathingState(state)["system.props.resp_vento_estado"])));
...
describe("Respiração do Vento — P0: dano chega ao contrato pendingDamage", () => {
  ...
  it("Vendaval: 3 ataques reais, ignora resistências, Exaustão única no N3+", () => {
    for (const [level, formula] of [[1, "2d4 + @fdv"], [2, "2d6 + @fdv"], [3, "2d8 + @fdv"], [4, "2d8 + @fdv + @dex"]]) {
    ...
    assert.equal(buildWindBreathingPlan("vento_06", 1, PROPS_BASE, {}).state.pendingDamage.uses, 3);
    const exhaustionN2 = buildWindBreathingPlan("vento_06", 2, PROPS_BASE, {}).patch["system.props.status_slayer_exaustao"];
    assert.equal(exhaustionN2, undefined);
    const exhaustionN3 = buildWindBreathingPlan("vento_06", 3, PROPS_BASE, {}).patch["system.props.status_slayer_exaustao"];
  ...
  it("Corte da Primeira Ventania: N1/N2 indisponíveis · cicatrizes consolidadas escalam", () => {
    ...
    const noScarN3 = buildWindBreathingPlan("vento_09", 3, { ...PROPS_BASE, resp_vento_estado: JSON.stringify({ scars: 0 }) }, {});
    assert.equal(noScarN3.state.pendingDamage.formula, "4d12"); // mínimo garantido max(1, scars)
    const threeScars = buildWindBreathingPlan("vento_09", 3, { ...PROPS_BASE, resp_vento_estado: JSON.stringify({ scars: 3 }) }, {});
    assert.equal(threeScars.state.pendingDamage.formula, "4d12 + 4d12 + 4d12");
    const fourScarsN4 = buildWindBreathingPlan("vento_09", 4, { ...PROPS_BASE, resp_vento_estado: JSON.stringify({ scars: 4 }) }, {});
  ...
  it("Tufão Idaten: requer N4 + DEX ≥4 · dano composto · Exaustão e campos de sangramento", () => {
    assert.equal(buildWindBreathingPlan("vento_10", 4, { ...PROPS_BASE, dex_display: 3 }, {}).ok, false);
    const plan = buildWindBreathingPlan("vento_10", 4, { ...PROPS_BASE, dex_display: 5, resp_vento_estado: JSON.stringify({ scars: 3 }) }, {});
    assert.equal(plan.ok, true);
