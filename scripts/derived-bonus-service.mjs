...
function metalFallback(props, channel) {
  const metal = String(props.metal_escolhido ?? "").trim().toLowerCase();
  const breathLevel = Math.max(0, parseLevel(props.nvl_respiracao_num));
  const values = {
    acerto: metal === "metal_dourada" ? 4 : 0,
    bloqueio: metal === "metal_preta" || metal === "metal_azul" ? (metal === "metal_preta" ? 4 : 3) : 0,
...
export function resolveSlayerDerivedBonuses(props = {}, context = {}) {
  const channels = Object.fromEntries(DERIVED_BONUS_CHANNELS.map((channel) => [channel, emptyChannel()]));
  const typedDamage = [];
  const allSources = [];
  ...
  const abilityValues = {
    ...
    percepcaoVisual: resolvedProp(props, "hab_percepcao_visual_bonus", abilityFallback(props, "percepcaoVisual")),
    percepcaoAuditiva: resolvedProp(props, "hab_percepcao_auditiva_bonus", abilityFallback(props, "percepcaoAuditiva")),
  ...
  const metalValues = {
    ...
    percepcaoVisual: resolvedProp(props, "metal_percepcao_visual_bonus", metalFallback(props, "percepcaoVisual")),
    danoFixo: resolvedProp(props, "metal_dano_bonus", metalFallback(props, "danoFixo")),
    iniciativa: resolvedProp(props, "metal_iniciativa_bonus", metalFallback(props, "iniciativa")),
  ...
  const poison = resolvedProp(props, "metal_dano_veneno", String(props.metal_escolhido ?? "").toLowerCase() === "metal_roxa" ? 3 : 0);
  if (poison) {
    const source = { channel: "danoTipado", value: poison, label: "Metal Roxo", origin: "Metal", types: ["envenenamento"] };
  ...
  for (const runtime of context.runtimeSources ?? []) {
    if (!runtime || runtime.channel === "danoTipado") {
      ...
      if (value) {
        const source = { channel: "danoTipado", value, label: runtime.label ?? "Efeito temporário", origin: runtime.origin ?? "Temporário", types: [...(runtime.types ?? [])] };
        typedDamage.push(source);
        allSources.push(source);
      }
      continue;
    }
    add({ ...runtime, label: runtime.label ?? "Efeito temporário", origin: runtime.origin ?? "Temporário" });
  }
...
export function derivedChannelForTest(test, modality = "") {
  const normalized = `${test} ${modality}`.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase();
...
export function derivedBonusSummary(resolved) {
  const labels = [
    ["Acerto", "acerto"], ["Bloqueio", "bloqueio"], ["Esquiva", "esquiva"],
    ["Percepção visual", "percepcaoVisual"], ["Percepção auditiva", "percepcaoAuditiva"],
    ["Dano", "danoFixo"],
  ];
  return labels.map(([label, key]) => `${label} ${resolved.channels[key].total >= 0 ? "+" : ""}${resolved.channels[key].total}`).join(" · ");
}
...
export async function openDerivedBonusAudit(options = {}) {
  if (!globalThis.game?.user?.isGM) return globalThis.ui?.notifications?.warn?.("Somente o GM pode auditar bônus derivados.");
  const actor = await resolveActor(options);
  if (!actor) return globalThis.ui?.notifications?.warn?.("Nenhum Actor encontrado para auditoria de bônus.");
  const resolved = resolveSlayerDerivedBonuses(actor.system?.props ?? {});
  const rows = resolved.allSources.length
    ? resolved.allSources.map((source) => `<tr><td>${escapeHtml(source.origin)}</td><td>${escapeHtml(source.label)}</td><td>${escapeHtml(source.channel)}</td><td>${source.value > 0 ? "+" : ""}${source.value}${source.types.length ? ` (${escapeHtml(source.types.join(", "))})` : ""}</td></tr>`).join("")
