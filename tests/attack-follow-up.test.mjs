...
let _rollResult = { total: 10, toMessage: async () => {}, dice: [{ results: [{ result: 1, active: true }] }] };
Roll.create = () => ({ evaluate: async () => _rollResult, dice: [{ results: [{ result: 1, active: true }] }] });
...
describe("attack-follow-up — pipeline Acerto → Dano (Problemas 1, 2 e 3)", () => {
  it("Problema 1 — Acerto confirmado com arma dispara o dano automaticamente, sem clique manual na arma", async () => {
    ...
    const chatFlavors = [];
    ChatMessage.create = async (data) => { chatFlavors.push(data?.flavor ?? ""); return data; };

    _rollResult = { total: 14, toMessage: async () => ({ id: "hit" }), dice: [{ results: [{ result: 10, active: true }] }] };
    _dialogReturn = [
      { mode: "normal", rollMode: "publicroll", bonusRaw: "", cdVal: 0, rollCount: 1, actionType: "ataque", weaponId: "weapon1", weaponProfileIndex: 0, weaponAttribute: "" }, // openHitDialog
      { hit: true, continue: false }, // openHitConfirmationDialog
      { chain: false }, // openChainFormDialog (Problema 2)
      { nome: "Nichirin", pdrGasto: 0, entradas: [{ dado: "1d10", fixo: 3, selAttrs: [], selTiposDano: ["cortante"], tipoAcao: "ataque" }], critical: false }, // openDamageDialog (rollWeaponItem -> rollDamage)
    ];
    ...
    assert.ok(chatFlavors.some((flavor) => flavor.includes("Total:")), `esperava um chat de dano automático; flavors: ${JSON.stringify(chatFlavors)}`);
  });
  ...
  it("Problema 2 — encadear outra Forma no diálogo pós-Acerto NÃO rola o dano da técnica atual", async () => {
    ...
    assert.ok(!chatFlavors.some((flavor) => flavor.includes("Total:")), `dano não deveria ter sido rolado ao encadear; flavors: ${JSON.stringify(chatFlavors)}`);
    assert.ok(warnings.some((message) => message.includes("Item não encontrado")), `esperava que useBreathForm tentasse resolver a próxima Forma; warnings: ${JSON.stringify(warnings)}`);
  });
  ...
  it("Problema 3 — técnica sem dado de dano próprio (ex.: 2ª Forma das Chamas) usa a arma equipada quando nenhuma foi escolhida no diálogo de Acerto", async () => {
    ...
    const formulas = [];
    Roll.create = (formula) => { formulas.push(formula); return { evaluate: async () => ({ total: 12, dice: [] }) }; };
    _dialogReturn = [
      { nome: "Nichirin", pdrGasto: 0, entradas: [{ dado: "2d6", fixo: 0, selAttrs: [], selTiposDano: ["cortante"], tipoAcao: "ataque" }], critical: false },
    ];
    ...
    await resolveAutoDamage({ actor, hitResult, techniqueLabel: "Chamas Céu em Chamas Ascendentes", techniqueEntradas: [] });

    assert.ok(formulas.some((formula) => formula.includes("2d6")), `esperava o dano da arma na fórmula final; formulas: ${JSON.stringify(formulas)}`);
  });
  ...
  it("Problema 3 (controle) — com MAIS de uma arma distinta, não adivinha: sem entrada, avisa e não inclui dano de arma nenhuma", async () => {
    ...
    const formulas = [];
    const warnings = [];
    Roll.create = (formula) => { formulas.push(formula); return { evaluate: async () => ({ total: 0, dice: [] }) }; };
    ui.notifications.warn = (message) => warnings.push(message);
    _dialogReturn = { nome: "Técnica", pdrGasto: 0, entradas: [] };

    const hitResult = { hits: 1, attempts: [{ hit: true, critical: false }], weapon: null };
    await resolveAutoDamage({ actor, hitResult, techniqueLabel: "Técnica sem dado próprio", techniqueEntradas: [] });

    assert.ok(!formulas.some((formula) => formula.includes("2d6") || formula.includes("2d8")), "não deveria adivinhar qual das duas armas usar");
    assert.ok(warnings.some((message) => message.includes("Adicione ao menos uma entrada")), "deveria avisar que falta informar o dano, em vez de escolher uma arma arbitrariamente");
  });
});
