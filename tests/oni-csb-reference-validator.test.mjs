...
const KNOWN_CSB_FUNCTIONS = new Set([
  "fallback", "max", "min", "switchCase", "equalText", "replace",
  "sum", "concat", "floor", "ceil", "round", "abs", "if", "and", "or", "not",
  "fetchFromParent", "isEmpty", "isNotEmpty",
]);
const RESERVED_WORDS = new Set(["entity", "linkedEntity", "true", "false", "null", "top", "fallback"]);

function collect(value, predicate, out = []) {
  if (Array.isArray(value)) { for (const e of value) collect(e, predicate, out); return out; }
...
describe("Validador de referências CSB — Oni (guarda contra props órfãs)", () => {
  it("nenhuma fórmula ${...}$ do template Oni referencia propriedade inexistente", async () => {
    ...
    for (const node of collect(source.system.body, (n) => typeof n.key === "string" && n.key !== "")) resolvable.add(node.key);
    for (const node of collect(source.system.header, (n) => typeof n.key === "string" && n.key !== "")) resolvable.add(node.key);
    ...
    const allFormulaSources = [
      ...
      ...collect(source.system.body, (n) => typeof n.visibilityFormula === "string" && n.visibilityFormula).map((n) => n.visibilityFormula),
      ...collect(source.system.body, (n) => typeof n.editableFormula === "string" && n.editableFormula).map((n) => n.editableFormula),
      ...collect(source.system.header, (n) => typeof n.value === "string").map((n) => n.value),
    ...
    for (const source_ of allFormulaSources) {
      for (const body of extractFormulaBodies(source_)) {
        for (const identifier of extractIdentifiers(body)) {
          if (resolvable.has(identifier) || KNOWN_CSB_FUNCTIONS.has(identifier)) continue;
    ...
    if (orphans.size > 0) {
      const report = [...orphans.entries()].map(([id, snippets]) => `  - "${id}" em: ${[...snippets].join(" | ")}`).join("\n");
      assert.fail(`Propriedades órfãs referenciadas em fórmulas CSB do template Oni (causariam undefined em runtime):\n${report}`);
    }
  });

  it("regressão direta: pdv_oni_ganho_nvl2..12 existem como componentes reais (não só na fórmula)", async () => {
    const source = JSON.parse(await readFile(new URL("../src/templates/actors/oni-template.json", import.meta.url), "utf8"));
    const keys = new Set(collect(source.system.body, (n) => typeof n.key === "string").map((n) => n.key));
    for (let level = 2; level <= 12; level += 1) {
      assert.ok(keys.has(`pdv_oni_ganho_nvl${level}`), `pdv_oni_ganho_nvl${level} deve existir como componente real`);
    }
  });
});
