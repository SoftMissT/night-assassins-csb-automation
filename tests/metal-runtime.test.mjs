...
describe("Runtime da Respiração do Metal", () => {
  ...
  it("lista apenas aliado sinérgico presente com PDR disponível", () => {
    ...
    const listed = metalHammerSynergyAllies(attacker, [attacker, eligible, empty, wrong, enemy], { isAlly: (candidate) => candidate.uuid !== "enemy" });
    assert.deepEqual(listed.map(({ uuid }) => uuid), ["eligible"]);
  ...
  it("calcula e cobra 1 PDR na chave canônica do aliado", () => {
    ...
    assert.deepEqual(spendMetalHammerSynergyPdr(ally).patch, { "system.props.pdr_slayer_gasto_valor": 4 });
    assert.equal(spendMetalHammerSynergyPdr(actor("empty", "Pedra", { pdr_slayer_gasto_valor: 10 })).ok, false);
  });

  it("gera dano de oportunidade somente no 1 natural seguido de acerto", () => {
    const state = buildMetalBreathingPlan("metal_02", 4, {}).state;
    assert.equal(buildInabalavelOpportunityDamage(state, { enemyNatural: 2, opportunityHit: true }).entries.length, 0);
    assert.equal(buildInabalavelOpportunityDamage(state, { enemyNatural: 1, opportunityHit: false }).entries.length, 0);
    assert.deepEqual(buildInabalavelOpportunityDamage(state, { enemyNatural: 1, opportunityHit: true }).entries, [
