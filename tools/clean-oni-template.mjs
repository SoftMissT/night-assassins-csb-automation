...
const LEGACY_DISPLAY_KEYS = new Set([
    'pdv_oni_total_valor',
    'pdv_oni_atual_valor_display',
    'pdk_oni_total_valor',
    'pdk_oni_atual_valor_display',
]);
const ONI_FORBIDDEN_UI_KEYS = new Set([
    ...FOLEGO_KEYS,
    ...MARCA_TEMP_KEYS,
    'vida_morte_oni_panel',
    'vida_morte_oni_titulo',
    'vida_morte_oni_estado',
    'vida_morte_oni_sucessos',
    'vida_morte_oni_falhas',
    'descanso_oni_gerenciar',
    'descanso_oni_dados',
...
function normalizeOniLedgerNode(node) {
    ...
    if (node.key === 'pdv_oni_total_valor')
        Object.assign(node, ledgerDisplayField(node.key, '${pdv_oni_total_conta}$', '#C1000C'));
    if (node.key === 'pdv_oni_atual_valor_display')
        Object.assign(node, ledgerDisplayField(node.key, '${pdv_oni_atual_num}$', '#C1000C'));
    if (node.key === 'pdk_oni_total_valor')
        Object.assign(node, ledgerDisplayField(node.key, '${pdk_oni_total_conta}$', '#B36CFF'));
    if (node.key === 'pdk_oni_atual_valor_display')
        Object.assign(node, ledgerDisplayField(node.key, '${pdk_oni_atual_num}$', '#B36CFF'));
    const labels = {
...
function ensureOniCombatLedger(t, combatTab) {
    ...
    const ordered = [
        ...
        normalizeOniLedgerNode(
            ensureNodeByKey(
                existingByKey,
                'pdv_oni_total_valor',
                ledgerDisplayField('pdv_oni_total_valor', '${pdv_oni_total_conta}$', '#C1000C')
            )
        ),
        ...
        normalizeOniLedgerNode(
            ensureNodeByKey(
                existingByKey,
                'pdv_oni_dano_tomado',
                ledgerNumberField('pdv_oni_dano_tomado', 'Dano Tomado')
            )
        ),
        normalizeOniLedgerNode(
            ensureNodeByKey(
                existingByKey,
                'pdv_oni_curado',
                ledgerNumberField('pdv_oni_curado', 'Curado')
            )
        ),
        normalizeOniLedgerNode(
            ensureNodeByKey(
                existingByKey,
                'pdv_oni_extra',
                ledgerNumberField('pdv_oni_extra', 'PDV Extra')
            )
        ),
        ...
        normalizeOniLedgerNode(
            ensureNodeByKey(
                existingByKey,
                'pdk_oni_total_valor',
                ledgerDisplayField('pdk_oni_total_valor', '${pdk_oni_total_conta}$', '#B36CFF')
            )
        ),
        ...
        normalizeOniLedgerNode(
            ensureNodeByKey(
                existingByKey,
                'pdk_oni_gasto_valor',
                ledgerNumberField('pdk_oni_gasto_valor', 'PDK Gasto')
            )
        ),
        normalizeOniLedgerNode(
            ensureNodeByKey(
                existingByKey,
                'pdk_oni_curado',
                ledgerNumberField('pdk_oni_curado', 'Recuperado')
            )
        ),
        normalizeOniLedgerNode(
            ensureNodeByKey(
                existingByKey,
                'pdk_oni_extra',
                ledgerNumberField('pdk_oni_extra', 'PDK Extra')
            )
        ),
    ];
    combatTab.contents.unshift(
        panel('recursos_oni_admin_panel', ordered, {
            title: 'PDV / PDK',
            flow: 'grid-5',
            cssClass: 'na-oni-ledger-panel',
        })
    );
}
...
function repairCurrentOniTemplate(t, tabbedPanel) {
    ...
    if (!combatTab || !configsTab)
        throw new Error('Abas Oni Combate/Configurações não encontradas.');
    combatTab.name = 'COMBATE';
    configsTab.name = 'CONFIGURAÇÕES';
    ensureOniCombatLedger(t, combatTab);
    t.system.templateSystemUniqueVersion =
        Math.max(1, Number(t.system.templateSystemUniqueVersion) || 0) + 1;
    return t;
}
...
function pruneByKeys(value, keys) {
    if (Array.isArray(value))
        return value.map((entry) => pruneByKeys(entry, keys)).filter((entry) => entry !== null);
...
function ensureLedgerIntegrity(t) {
    ...
    if (storagePanel) {
        ...
        for (let level = 2; level <= 12; level += 1) {
            ...
            if (!existingKeys.has(key)) {
                storagePanel.contents.push(
                    numberField(
                        key,
                        `PDV ganho Nv.${level} (auto)`,
                        'Rolado automaticamente uma única vez ao atingir o nível.'
                    )
                );
            }
        }
        for (const key of ['hab_esquiva_bonus', 'hab_bloqueio_bonus', 'hab_acerto_bonus']) {
            if (!existingKeys.has(key)) {
                storagePanel.contents.push(
                    numberField(
                        key,
                        `Bônus de ${key.replace('hab_', '').replace('_bonus', '')} (Especialização/Origem)`,
                        'Bônus permanente somado ao teste correspondente.'
                    )
                );
            }
        }
        ...
        for (const entry of t.system.hidden ?? []) {
            if (typeof entry.value === 'string')
                entry.value = entry.value.replaceAll(
    ...
    if (!hiddenNames.has('rank_especializacao_oni')) {
        t.system.hidden = [
            ...(t.system.hidden ?? []),
            { name: 'rank_especializacao_oni', value: RANK_ESPECIALIZACAO_FORMULA },
        ];
    }
}
...
export function cleanOniTemplate(source) {
    ...
    t.system.hidden = (t.system.hidden ?? []).filter(
        (entry) => !removedHiddenNames.has(entry.name)
    );
    ...
    const {
        perfil_oni_tab: perfil,
        pericias_tab: pericias,
        combat_oni_tab: combat,
        inventario_oni_tab: inventario,
        notas_oni_tab: notas,
        configs_tab: configs,
    } = oldTabs;
    ...
    combat.contents.push(
        itemContainer(
            'inventario_oni_kekkijutsus',
            'Kekkijutsu',
            'kekkijutsu',
            'Técnicas de sangue conhecidas pelo Oni.'
        )
    );
    ...
    const especializacaoSection = [
        ...
        panel(
            'especializacao_oni_resumo_panel',
            [
                label(
                    "Especialização atual: ${fallback(oni_especializacao_id,'oni_especializacao_escolha')}$",
                    { size: 'na-sheet-size-md' }
                ),
                label("Rank da Especialização: ${fallback(rank_especializacao_oni,'—')}$", {
                    size: 'na-sheet-size-md',
                }),
            ],
    ...
    const origemProgressaoSection = [
        ...
        panel(
            'origem_oni_resumo_panel',
            [
                label("Origem: ${fallback(origem_dropdown,'origem_oni_escolha')}$", {
                    size: 'na-sheet-size-md',
                }),
            ],
