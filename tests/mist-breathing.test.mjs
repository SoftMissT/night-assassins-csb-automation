...
import {
    ...
    consumeMistPatternBenefit,
    consumeMistPending,
    grantMistPattern,
    mistPatternCount,
    normalizeMistBreathingState,
    parseMistBreathingState,
    resolveEightLayersResult,
    resolveMistFormula,
    resolveMistReduction,
    resolveMistStigmaStunOnHit,
    resolveMistReflectionRecoveryAvailable,
    tickMistBreathing,
...
function threePatterns() {
    ...
    for (const pattern of ['cyclone', 'stigma', 'reflection'])
        state = grantMistPattern(state, pattern);
    return state;
}
...
describe('Respiração da Névoa — dados', () => {
    it('cataloga oito Formas com níveis, custos e ações oficiais', () => {
        ...
        assert.deepEqual(
            MIST_FORMS.map((form) => form.order),
            [1, 2, 3, 4, 5, 6, 7, 8]
        );
...
describe('Respiração da Névoa — estado dos Padrões (earned x benefitAvailable)', () => {
    it('normaliza estado legado (booleano) sem quebrar Actors existentes', () => {
        ...
        assert.deepEqual(legacy.patterns.cyclone, {
            earned: true,
    ...
    it('normaliza estado ausente/corrompido para o schema vazio', () => {
        ...
        assert.equal(
            normalizeMistBreathingState('{ isso não é JSON').patterns.cyclone.earned,
            false
        );
    ...
    it('grantMistPattern marca earned E benefitAvailable; consumeMistPatternBenefit preserva earned', () => {
        ...
        assert.deepEqual(state.patterns.cyclone, {
            earned: true,
            benefitAvailable: true,
            turnsRemaining: 1,
        });
        ...
        assert.equal(
            state.patterns.cyclone.earned,
            true,
            'earned nunca é apagado ao consumir o benefício'
        );
    ...
    it('mistPatternCount conta earned, não benefitAvailable', () => {
        ...
        state = consumeMistPatternBenefit(state, 'cyclone');
        assert.equal(
            mistPatternCount(state),
            1,
            'benefício gasto não reduz a contagem de Padrões conquistados'
        );
    });
    ...
    it('os três Padrões coexistem de forma independente (não se sobrescrevem)', () => {
        ...
        assert.equal(
            mistPatternCount(state),
            3,
            'consumir o benefício de um Padrão não afeta os outros dois'
        );
    });
});
...
describe('1ª Forma — Céu Suspenso', () => {
    ...
    it('Combo A: uma técnica intermediária não consome o pendente; só o consumo explícito de hit/damage apaga', () => {
        ...
        assert.equal(
            state.pendingDamage.formula,
            '4 + 2',
            "consumir só 'hit' não apaga o dano pendente"
        );
...
describe('2ª Forma — Névoa de Oito Camadas', () => {
    ...
    for (const [hits, expectedMode, expectedRolls] of [
        [0, 'none', 0],
        [1, 'weapon-per-hit', 1],
        [2, 'weapon-per-hit', 2],
    ]) {
    ...
    for (const [level, hits, expectedFormula] of [
        [1, 3, '5d6'],
        [2, 4, '6d6'],
        [3, 3, '8d6'],
        [4, 5, '10d6'],
    ...
    it('Combo B: 5/5 acertos concede Ciclone da Névoa (earned + benefício), Padrão permanece após o uso', () => {
        ...
        const freePlan = buildMistBreathingPlan(
            'nevoa_01',
            4,
            { ...props, resp_nevoa_estado: five.state },
            { useCycloneFree: true }
        );
        assert.equal(freePlan.cost, 0, 'Ciclone isenta o custo BASE da próxima técnica');
        assert.equal(
            freePlan.state.patterns.cyclone.earned,
            true,
            'Padrão Ciclone permanece após consumir o benefício'
        );
        assert.equal(
            freePlan.state.patterns.cyclone.benefitAvailable,
            false,
            'benefício gasto, não pode ser reusado'
        );
    });

    it('Ciclone não cobre custos extras opcionais (4ª/5ª) — só o custo BASE', () => {
        const cycloneState = grantMistPattern({}, 'cyclone');
        const plan = buildMistBreathingPlan(
            'nevoa_04',
            3,
            { ...props, resp_nevoa_estado: cycloneState },
            {
                useCycloneFree: true,
                advantageAttack: true,
                suppressResistance: true,
                suppressAttribute: 'dex',
            }
        );
        assert.equal(
            plan.cost,
            1,
            'custo base (3) isento + apenas o +1 PDR opcional da Anulação de Resistências'
        );
    });
});
...
describe('3ª Forma — Expansão de Névoa', () => {
    ...
    it('N1/N2: 1d6 + Nível de Exterminador; N3/N4: soma SAB também', () => {
        assert.equal(
            buildMistBreathingPlan('nevoa_03', 1, props).state.incomingReduction.formula,
            '1d6 + 8'
        );
        assert.equal(
            buildMistBreathingPlan('nevoa_03', 2, props).state.incomingReduction.formula,
            '1d6 + 8'
        );
        assert.equal(
            buildMistBreathingPlan('nevoa_03', 3, props).state.incomingReduction.formula,
            '1d6 + 8 + 4'
        );
        assert.equal(
            buildMistBreathingPlan('nevoa_03', 4, props).state.incomingReduction.formula,
            '1d6 + 8 + 4'
        );
    });

    it('resultado maior → anula; resultado menor → subtrai', () => {
        assert.deepEqual(resolveMistReduction(20, 7), {
            incoming: 20,
            reduction: 7,
            negated: false,
            finalDamage: 13,
...
describe('4ª Forma — Corte de Advecção / Fecha Neblinada', () => {
    ...
    it('dano SUBSTITUI (nunca soma) e escala 3d6/4d6/5d6/6d6 por nível', () => {
        for (const [level, expected, cost] of [
            [1, '3d6', 2],
            [2, '4d6', 2],
            [3, '5d6', 3],
            [4, '6d6', 3],
        ]) {
            const plan = buildMistBreathingPlan('nevoa_04', level, props, {
                advantageAttack: true,
            });
    ...
    it('Sinergia: +1 PDR opcional e Anulação de Resistências com duração por DEX OU FOR (escolha do usuário)', () => {
        const byDex = buildMistBreathingPlan('nevoa_04', 4, props, {
            advantageAttack: true,
            suppressResistance: true,
            suppressAttribute: 'dex',
        });
        ...
        const byFor = buildMistBreathingPlan('nevoa_04', 4, props, {
            advantageAttack: true,
            suppressResistance: true,
            suppressAttribute: 'for',
        });
    ...
    it('sem sinergia escolhida, não paga o PDR extra nem aplica Anulação de Resistências', () => {
        const plan = buildMistBreathingPlan('nevoa_04', 4, props, {
            advantageAttack: true,
            suppressResistance: false,
        });
        assert.equal(plan.cost, 3);
...
describe('5ª Forma — Mar de Nuvens Neblinadas', () => {
    ...
    it('Combo D: pagar o DOBRO do custo TOTAL concede Reflexão; Padrão permanece após usar a Recuperação com Vantagem', () => {
        ...
        assert.equal(
            recovery.state.patterns.reflection.earned,
            true,
            'Padrão permanece após consumir a Recuperação com Vantagem'
        );
...
describe('6ª Forma — Névoa sob o Luar', () => {
    it('declaração cobra 2 PDR mesmo se o teste de DEX falhar (custo por declaração, não por sucesso)', () => {
        ...
        assert.equal(
            plan.ok,
            true,
            "a Forma continua 'ok' — o custo de declaração já ocorreu, sem criar dano"
        );
        assert.equal(plan.cost, 2);
    ...
    it('sucesso: +SAB em todos os ataques do turno (nextHit.bonus = SAB)', () => {
        const plan = buildMistBreathingPlan('nevoa_06', 2, props, {
            dexCheckPassed: true,
            extraAttacks: 0,
        });
    ...
    it('cadeia de ataques: cada ataque extra cobra +1 PDR (não pré-cobra tudo de uma vez conceitualmente)', () => {
        const plan = buildMistBreathingPlan('nevoa_06', 4, props, {
            dexCheckPassed: true,
            extraAttacks: 3,
        });
        assert.equal(
            plan.cost,
            2 + 3,
            '2 PDR de declaração + 1 PDR por cada um dos 3 ataques extras'
        );
    ...
    it('Combo E: com os 3 Padrões conquistados, Colapso ativa — +SAB no dano de cada acerto', () => {
        ...
        const plan = buildMistBreathingPlan(
            'nevoa_06',
            4,
            { ...props, resp_nevoa_estado: state },
            { dexCheckPassed: true, extraAttacks: 0 }
        );
        assert.equal(plan.state.collapse, true);
        assert.equal(plan.state.pendingDamage.formula, '@sab');
        assert.equal(
            mistPatternCount(plan.state),
            3,
            'Colapso NÃO consome os 3 Padrões (decisão pendente, mantido conservador)'
        );
    });

    it('crítico soma FDV no Acerto e no Dano SOMENTE durante Colapso, e só naquele ataque', () => {
        const state = threePatterns();
        const collapsePlan = buildMistBreathingPlan(
            'nevoa_06',
            4,
            { ...props, resp_nevoa_estado: state },
            { dexCheckPassed: true, extraAttacks: 0 }
        ...
        const noPatterns = buildMistBreathingPlan('nevoa_06', 4, props, {
            dexCheckPassed: true,
            extraAttacks: 0,
        });
        assert.equal(noPatterns.state.collapse, false);
        assert.equal(
            noPatterns.state.nextHit.criticalBonus,
            0,
            'sem Colapso, o crítico não ganha bônus de FDV'
        );
        assert.equal(
            noPatterns.state.pendingDamage,
            undefined,
            'sem Colapso, a 6ª Forma não adiciona dano extra por acerto'
        );
    });
});
...
describe('7ª Forma — Neblina', () => {
    ...
    it('bônus por nível em Acerto/Dano/Esquiva/Bloqueio: +2/+3/+4', () => {
        assert.equal(
            buildMistBreathingPlan('nevoa_07', 2, props, { opposedPassed: true }).state.fog.bonus,
            2
        );
        assert.equal(
            buildMistBreathingPlan('nevoa_07', 3, props, { opposedPassed: true }).state.fog.bonus,
            3
        );
        assert.equal(
            buildMistBreathingPlan('nevoa_07', 4, props, { opposedPassed: true }).state.fog.bonus,
            4
        );
    });
...
describe('8ª Forma — Ofuscamento', () => {
    it('indisponível no Nível 1, 5 turnos, 7 PDR em N2/N3/N4', () => {
        for (const level of [2, 3, 4]) {
            const plan = buildMistBreathingPlan('nevoa_08', level, props, {
                targetUuid: 'Actor.Enemy',
                allyUuid: 'Actor.Ally',
            });
    ...
    it('N2: -2 no Acerto do inimigo contra usuário/aliado + imunidade a Exaustão', () => {
        const plan = buildMistBreathingPlan('nevoa_08', 2, props, {
            targetUuid: 'Actor.Enemy',
            allyUuid: 'Actor.Ally',
        });
    ...
    it('N3: mantém N2 + target-specific +2 Acerto contra o inimigo ofuscado', () => {
        const plan = buildMistBreathingPlan('nevoa_08', 3, props, {
            targetUuid: 'Actor.Enemy',
            allyUuid: 'Actor.Ally',
        });
        assert.equal(plan.state.obfuscation.hitPenalty, -2);
    ...
    it('N4: mantém anteriores + normaliza dano crítico (não vira erro)', () => {
        const plan = buildMistBreathingPlan('nevoa_08', 4, props, {
            targetUuid: 'Actor.Enemy',
            allyUuid: 'Actor.Ally',
        });
        assert.equal(plan.state.obfuscation.criticalImmunity, true);
        assert.equal(
            plan.state.dazzle.criticalImmunity,
            true,
            "campo legado 'dazzle' sincronizado para hit-service/damage-service"
        );
    });
});
...
describe('Combo F — Neblina + Lua: bônus independentes coexistem', () => {
    it('Neblina (+3 Acerto/Dano N3) e Lua (+SAB Acerto) não se substituem', () => {
        ...
        const luar = buildMistBreathingPlan(
            'nevoa_06',
            3,
            { ...props, resp_nevoa_estado: state },
            { dexCheckPassed: true, extraAttacks: 0 }
...
describe('Combo G — Ofuscamento + reação: estados independentes', () => {
    it('usar Expansão ou Mar de Nuvens não apaga o Ofuscamento ativo', () => {
        let state = buildMistBreathingPlan('nevoa_08', 4, props, {
            targetUuid: 'Actor.Enemy',
            allyUuid: 'Actor.Ally',
        }).state;
        const expansion = buildMistBreathingPlan(
            'nevoa_03',
            4,
            { ...props, resp_nevoa_estado: state },
            {}
        ...
        const mistSea = buildMistBreathingPlan(
            'nevoa_05',
            4,
            { ...props, resp_nevoa_estado: expansion.state },
            {}
        );
...
describe('Estados simultâneos — sem shallow-copy/aliasing entre Formas', () => {
    it('consumir/tickar um efeito não muta os demais estados aninhados por referência', () => {
        ...
        state = buildMistBreathingPlan(
            'nevoa_07',
            4,
            { ...props, resp_nevoa_estado: state, car_display: 5 },
            { opposedPassed: true }
        ).state;
        state = buildMistBreathingPlan(
            'nevoa_08',
            4,
            { ...props, resp_nevoa_estado: state },
            { targetUuid: 'Actor.Enemy', allyUuid: 'Actor.Ally' }
        ).state;
        state = buildMistBreathingPlan(
            'nevoa_05',
            2,
        ...
        const skySuspended = buildMistBreathingPlan('nevoa_01', 3, {
            ...props,
            resp_nevoa_estado: state,
        }).state;
        ...
        const ticked = normalizeMistBreathingState(
            tickMistBreathing(mistStateJson(skySuspended))['system.props.resp_nevoa_estado']
        );
        assert.equal(ticked.fog.turns, snapshotFogTurns - 1);
        assert.equal(ticked.obfuscation.turns, snapshotObfTurns - 1);
        assert.equal(
            mistPatternCount(ticked),
            snapshotPatterns,
            'earned dos 3 Padrões não muda só por tickar'
        );
        assert.equal(
            ticked.pendingDamage.formula,
            skySuspended.pendingDamage.formula,
            'Céu Suspenso pendente não é afetado pelo tick de Neblina/Ofuscamento'
        );
        assert.equal(
            ticked.nextHit.advantage,
            true,
            'Vantagem do Mar de Nuvens sobrevive ao tick de outras Formas'
        );
...
describe('clearMistBreathingState — combate encerrado', () => {
    it('limpa efeitos de combate e reseta os Padrões conquistados (decisão do Operador)', () => {
        ...
        assert.equal(
            mistPatternCount(cleared),
            0,
            'Padrões resetam ao fim do combate, mesmo padrão de Quebra/Esquentar'
        );
