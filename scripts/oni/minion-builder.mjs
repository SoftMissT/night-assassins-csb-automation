...
const TYPES_BY_ID = new Map(
    Object.entries(minionCatalog.types).map(([id, t]) => [id, Object.freeze(t)])
);
    ...
    return Object.freeze({
        ...
        attack: attackDef
            ? Object.freeze({
                  ...
                  testAttribute: attackDef.testAttribute,
              })
            : null,
        trait: traitDef
            ? Object.freeze({
...
export function recommendedCount(slayerCount = 1, danger = 'media') {
    ...
    const dangerKey = ['facil', 'media', 'perigosa'].includes(
        String(danger).trim().toLocaleLowerCase('pt-BR')
    )
        ? String(danger).trim().toLocaleLowerCase('pt-BR')
        : 'media';
