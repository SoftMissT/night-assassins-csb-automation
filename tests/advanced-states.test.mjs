...
describe('Lâmina Carmesim — regras puras', () => {
    it('descreve os três métodos de Ignição', () => {
        assert.deepEqual(laminaIgnitionProfile('sangue'), {
            method: 'sangue',
            acao: 'especial',
            custoPdv: '1d4',
            rastro: 1,
        ...
        assert.deepEqual(laminaIgnitionProfile('atrito'), {
            method: 'atrito',
            acao: 'especial',
            custoPdr: 3,
            rastro: 2,
    ...
    it('escala o Rastro de Calor 1 a 4', () => {
        assert.deepEqual(laminaRastroEffects(1), {
            solarDice: '1d4',
            cauterizacao: 'ateFimProximoTurnoOni',
        });
        assert.deepEqual(laminaRastroEffects(2), {
            solarDice: '1d6',
            cauterizacao: '2rodadas',
            movimento: { cd: 14, perdeMetros: 3 },
        });
        assert.deepEqual(laminaRastroEffects(3), {
            solarDice: '2d6',
            cauterizacao: '3rodadas',
            solarAoBloquear: '1d6',
        });
        assert.deepEqual(laminaRastroEffects(4), {
            solarDice: '3d6',
...
describe('Estado Altruísta — regras puras', () => {
    ...
    it('primeiro ataque por rodada: sem reação e +2 contra leitores de intenção', () => {
        assert.deepEqual(altruistaFirstAttack({ ativo: false }), {
            semReacao: false,
            bonusAcerto: 0,
        });
        assert.deepEqual(altruistaFirstAttack({ ativo: true, inimigoLeIntencao: false }), {
            semReacao: true,
            bonusAcerto: 0,
        });
        assert.deepEqual(altruistaFirstAttack({ ativo: true, inimigoLeIntencao: true }), {
            semReacao: true,
            bonusAcerto: 2,
        });
    });
    ...
    it('descreve o Corte Sem Ego e seus custos posteriores', () => {
        ...
        assert.deepEqual(corte.pos, {
            pdr: 0,
            exaustao: 2,
            fimEstado: true,
            travaAteDescansoN1: true,
        });
    });
});
...
describe('persistência e normalização', () => {
    ...
    it('lerAlvo e alvoLido operam por UUID', () => {
        let state = lerAlvo(defaultAdvancedStates(), {
            alvoUuid: 'Actor.abcd',
            alvoNome: 'Oni Carmesim',
        });
    ...
    it('saveAdvancedStates persiste dados e resumo no Actor', async () => {
        ...
        assert.deepEqual(
            readAdvancedStates({
                estados_slayer_dados: updates['system.props.estados_slayer_dados'],
            }),
...
describe('transições de estado', () => {
    ...
    it('ativarLamina valida nível, cena e recursos', () => {
        let state = defaultAdvancedStates();
        const baixoNivel = ativarLamina(state, {
            method: 'atrito',
            cenaOk: true,
            pdr: 10,
            level: 9,
        ...
        const pressaoSemCena = ativarLamina(state, {
            method: 'pressao',
            cenaOk: false,
            pdr: 10,
            level: 11,
    ...
    it('Pressão devolve teste VIT CD 16 e trava o método na falha', () => {
        let state = defaultAdvancedStates();
        const pressao = ativarLamina(state, {
            method: 'pressao',
            cenaOk: true,
            pdr: 10,
            level: 11,
        ...
        const retry = ativarLamina(resultado.state, {
            method: 'pressao',
            cenaOk: true,
            pdr: 10,
            level: 11,
...
describe('processamento de rodada e combate', () => {
    it('início de turno sobe o Rastro, expira o Foco e reseta o Corte Limpo', () => {
        ...
        assert.equal(
            inicio.messages.some((msg) => msg.includes('Rastro 3')),
            true
        );
    });
    ...
    it('6 estresses provocam Colapso Carmesim no fim do turno', () => {
        ...
        assert.equal(
            fim.messages.some((msg) => msg.includes('Colapso')),
            true
        );
    });
