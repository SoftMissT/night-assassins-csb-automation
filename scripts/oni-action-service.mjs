...
export function parseOniActionState(value) {
  ...
  if (typeof raw === "string") {
    ...
    catch (_) { return defaultOniActionState(); }
  }
...
export function oniActionMaximums(props = {}) {
  const bonus = (key) => Math.max(0, 1 + Math.trunc(parseNumber(props[`acoes_oni_${key}_bonus`])));
  return {
    movimento: bonus("movimento"), ataque: bonus("ataque"), especial: bonus("especial"),
    unica: 1, reacao: bonus("reacao"), lendaria: Math.max(0, Math.trunc(parseNumber(props.acoes_oni_lendaria_maximo))),
  };
}
...
function requiredCounters(types) {
  ...
  for (const key of new Set(types.filter(Boolean))) {
    if (key === "completa") { required.turn.movimento += 1; required.turn.ataque += 1; }
...
export async function consumeOniActions(actor, types, { update = true } = {}) {
  ...
  for (const key of TURN_KEYS) if (state.turn[key] + required.turn[key] > maximums[key]) return { ok: false, reason: `${TIPOS_ACAO.find((entry) => entry.key === key)?.label ?? key} indisponivel.` };
  for (const key of ROUND_KEYS) if (state.round[key] + required.round[key] > maximums[key]) return { ok: false, reason: `${TIPOS_ACAO.find((entry) => entry.key === key)?.label ?? key} indisponivel.` };
  ...
  const summary = [...TURN_KEYS, ...ROUND_KEYS].map((key) => {
    const used = state[ROUND_KEYS.includes(key) ? "round" : "turn"][key];
    return `${key.toUpperCase()} ${Math.max(0, maximums[key] - used)}/${maximums[key]}`;
  }).join(" · ");
  const patch = { "system.props.acoes_oni_dados": JSON.stringify(state), "system.props.acoes_oni_resumo": summary };
