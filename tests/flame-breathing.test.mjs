...
describe('Respiração das Chamas', () => {
    ...
    it('aplica os patamares substitutivos de Fogo Fátuo', () => {
        assert.deepEqual(flameWeaponTier(0), {
            heat: 0,
            hit: 0,
            weaponDamage: 0,
            techniqueDie: '',
    ...
    it('bloqueia formas indisponíveis e requisito de Cauterizar', () => {
        ...
        assert.equal(
            buildFlameBreathingPlan('chamas_07', 1, { resp_chamas_estado: '{"weaponHeat":5}' }).ok,
            true
        );
    });
    ...
    it('consome modificadores separadamente', () => {
        const state = parseFlameBreathingState({
            nextHit: { bonus: 2 },
            pendingDamage: { formula: '4d10', uses: 1 },
        });
    ...
    it('Ignição e Fogo Fátuo 60 causam dano por turno', () => {
        const tick = tickFlameBreathing({
            weaponHeat: 60,
            ignition: { turns: 1, damageBonus: 8, selfDamage: 5 },
        });
        assert.deepEqual(
            tick.events.map((event) => event.amount),
            [5, 2]
        );
    ...
    it('Ondulação prepara e consome interceptação para outro aliado', () => {
        const plan = buildFlameBreathingPlan('chamas_04', 4, {
            resp_chamas_estado: JSON.stringify({ weaponHeat: 30 }),
        });
        const prepared = buildFlameInterception(plan.state, {
            interceptorUuid: 'Actor.Fire',
            protectedUuid: 'Actor.Ally',
        });
        assert.equal(prepared.ok, true);
        assert.equal(
            consumeFlameInterception(prepared.flag, 'Actor.Ally').interceptorUuid,
            'Actor.Fire'
        );
        assert.equal(consumeFlameInterception(prepared.flag, 'Actor.Other').intercepted, false);
        assert.equal(
            buildFlameInterception(plan.state, {
                interceptorUuid: 'Actor.Fire',
                protectedUuid: 'Actor.Fire',
            }).ok,
            false
        );
    });
    ...
    it('a Forma soma Esquentar somente na arma sincronizada e o fim do combate limpa todas', () => {
        ...
        const plan = buildFlameBreathingPlan(
            'chamas_03',
            1,
            { resp_chamas_estado: JSON.stringify(state) },
            {
        ...
        const cleared = parseFlameBreathingState(
            JSON.parse(clearFlameBreathingState()['system.props.resp_chamas_estado'])
        );
...
describe('Respiração das Chamas combos e regras da missão', () => {
    it('COMBO OBRIGATÓRIO 1ª N2 + 2ª N2: rider aplicado à 2ª e ataque adicional preservado', () => {
        ...
        const plan2 = buildFlameBreathingPlan('chamas_03', 2, {
            resp_chamas_estado: JSON.stringify(plan1.state),
        });
    ...
    it('combo 1ª N3 + 2ª: ataque adicional recebe +1d4 com rolagem própria', () => {
        ...
        const plan2 = buildFlameBreathingPlan('chamas_05', 1, {
            resp_chamas_estado: JSON.stringify(plan1.state),
        });
        assert.equal(plan2.state.pendingDamage.comboRider.formula, '2d4');
    ...
    it('resolveFlameRengokuAllies filtra respiração compatível, PDR e o próprio usuário', () => {
        ...
        assert.deepEqual(
            eligible.map((ally) => ally.uuid),
            ['Actor.Amor', 'Actor.Magma']
        );
    ...
    it('Tigre Ardente: rolagem única, vantagem com 30+ e Exaustão N4 acima de 30 de dano', () => {
        ...
        const high = buildFlameBreathingPlan('chamas_05', 4, {
            resp_chamas_estado: JSON.stringify({ weaponHeat: 30 }),
        });
    ...
    it('Cauterizar exige 5 de Esquentar mas não os consome', () => {
        const plan = buildFlameBreathingPlan('chamas_07', 4, {
            resp_chamas_estado: JSON.stringify({ weaponHeat: 7 }),
        });
        assert.equal(plan.ok, true);
...
describe('Chamas fila de efeitos pendentes (combos encadeados)', () => {
    it('D · encadeado 1ª N3 → 2ª → 5ª → ataque adicional preserva o +1d4', () => {
        const plan1 = buildFlameBreathingPlan('chamas_02', 3, {});
        const state2 = buildFlameBreathingPlan('chamas_03', 2, {
            resp_chamas_estado: JSON.stringify(plan1.state),
        }).state;
        // 2ª acerta e consome o rider principal
        const after2 = consumeFlamePending(consumeFlamePending(state2, { hit: true }), {
            damage: true,
        });
        ...
        const state3 = buildFlameBreathingPlan('chamas_05', 2, {
            resp_chamas_estado: JSON.stringify(after2),
        }).state;
        ...
        const after3 = consumeFlamePending(consumeFlamePending(state3, { hit: true }), {
            damage: true,
        });
        ...
        const afterExtra = consumeFlamePending(consumeFlamePending(after3, { hit: true }), {
            damage: true,
        });
        const extraEffect = afterExtra.pendingEffects.find(
            (effect) => effect.id === 'unknown_fire_extra_attack'
        );
    ...
    it('F · consumir o rider principal não apaga extra attack, ignição nem block', () => {
        ...
        const extra = afterDamage.pendingEffects.find(
            (effect) => effect.id === 'unknown_fire_extra_attack'
        );
        ...
        assert.equal(
            afterExtra.pendingEffects.find((effect) => effect.id === 'unknown_fire_extra_attack')
                .used,
            true
        );
    ...
    it('G · estado legado (flameOne/pendingDamage sem fila) é normalizado sem quebrar', () => {
        ...
        const plan = buildFlameBreathingPlan('chamas_03', 2, {
            resp_chamas_estado: JSON.stringify(legacy),
        });
        assert.equal(plan.ok, true);
        assert.equal(plan.state.pendingDamage.comboRider.formula, '2d4');
        const queue = plan.state.pendingEffects;
        assert.ok(
            queue.some((effect) => effect.id === 'unknown_fire_main' && effect.formula === '2d4')
        );
        assert.ok(
            queue.some(
                (effect) =>
                    effect.id === 'unknown_fire_extra_attack' && effect.damageBonus === '1d4'
            )
        );
        const afterCombo = consumeFlamePending(consumeFlamePending(plan.state, { hit: true }), {
            damage: true,
        });
    ...
    it('H · Ignição + 1ª + 2ª coexistem sem sobrescrita', () => {
        ...
        const plan1 = buildFlameBreathingPlan('chamas_02', 2, {
            resp_chamas_estado: JSON.stringify(ignitionState),
        });
        const plan2 = buildFlameBreathingPlan('chamas_03', 2, {
            resp_chamas_estado: JSON.stringify(plan1.state),
        });
        ...
        assert.equal(plan2.state.pendingDamage.critical, true);
        assert.equal(plan2.state.pendingDamage.comboRider.formula, '2d4');
        const resolved = consumeFlamePending(consumeFlamePending(plan2.state, { hit: true }), {
            damage: true,
        });
    ...
    it('C · 1ª N4: ataque principal e adicional recebem +2d4 cada', () => {
        ...
        const plan2 = buildFlameBreathingPlan('chamas_03', 1, {
            resp_chamas_estado: JSON.stringify(plan1.state),
        });
        assert.equal(plan2.state.pendingDamage.comboRider.formula, '2d4');
        const afterCombo = consumeFlamePending(consumeFlamePending(plan2.state, { hit: true }), {
            damage: true,
        });
    ...
    it('E · nada da fila sobrevive ao fim do turno', () => {
        ...
        const tick = tickFlameBreathing({
            ...plan1.state,
            pendingEffects: plan1.state.pendingEffects,
        });
    ...
    it('DECISÃO DO OPERADOR · 1ª + Tormenta: rider integra a técnica (rolagem única), aplicado por alvo', () => {
        const plan1 = buildFlameBreathingPlan('chamas_02', 2, {});
        const stormPlan = buildFlameBreathingPlan('chamas_06', 3, {
            resp_chamas_estado: JSON.stringify(plan1.state),
        });
        ...
        assert.ok(
            stormPlan.state.pendingEffects.find(
                (effect) => effect.id === 'unknown_fire_main' && !effect.consumed
            )
        );
    });
});
