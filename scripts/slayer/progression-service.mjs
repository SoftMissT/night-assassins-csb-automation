...
function boundedLevel(raw) {
  ...
  if (!Number.isFinite(level) || level < 1 || level > 14) throw new RangeError("O nível Slayer deve estar entre 1 e 14.");
  return level;
}
...
export function resolveSlayerProgression(rawLevel) {
  const level = boundedLevel(rawLevel);
  const milestones = [];
  for (let current = 1; current <= level; current += 1) milestones.push(slayerProgressionAtLevel(current));
  const current = milestones.at(-1);
  const unlockedEvents = milestones.flatMap((entry) => entry.events.map((event) => ({ level: entry.level, event })));
  const unlockedClassRanks = CLASS_RANK_ORDER.filter((rank) => milestones.some((entry) => entry.classRank === rank));
  return {
    level,
    rank: current.rank,
