...
test('template Slayer preserva as abas oficiais Combate e Config/Dados', () => {
    ...
    function walk(node) {
        ...
        if (!tabbedPanel && node.type === 'tabbedPanel') {
            tabbedPanel = node;
        }
...
test('template Slayer preserva o dropdown que escolhe DEX ou FOR para Acerto', () => {
    ...
    assert.deepEqual(
        ...
        [
            {
                key: 'acerto_label_escolha',
                value: 'Escolha',
            },
            {
                key: 'acerto_label_dex',
                value: 'DEX',
            },
            {
                key: 'acerto_label_for',
                value: 'FOR',
            },
        ]
    );
...
test('nível 14 oferece escolha exclusiva entre bônus de PDV e PDR nos hidden totals', () => {
    ...
    function walk(node) {
        ...
        if (node.key === 'nvl_14_bonus_choice') {
            choice = node;
        }
...
test('template Slayer tem pelo menos 35 botões com macros estáveis', () => {
    ...
    function walk(node) {
        ...
        if (node.type === 'label' && node.rollMessage) {
            buttons.push(node);
        }
...
test('template Slayer tem itemContainer para armas e Formas', () => {
    ...
    function walk(node) {
        ...
        if (node.type === 'itemContainer') {
            containers.set(node.key, node);
        }
    ...
    /*
     * Armas normais e especiais compartilham o mesmo ItemContainer.
     *
     * Não usamos mais templateFilter com NAWeaponTpl00001 porque:
     * - existem dois templates de arma;
     * - o CSB pode remapear IDs no World;
     * - inventario_categoria é o contrato semântico estável.
     */
    ...
    /*
     * Formas de Respiração continuam usando o template específico.
     */
...
test('template Slayer tem hidden com atributos display', () => {
    ...
    const hiddenNames = template.system.hidden.map((entry) => entry.name);
