...
export async function applyInitialMark(actor) {
    ...
    if (!hasLevel1) {
        ui.notifications?.warn?.(
            'Atributos do nível 1 precisam ser concluídos antes de aplicar a Marca do Destino.'
        );
        return false;
    }
...
export async function upgradeMarkAtLevelSix(actor) {
    const props = actor.system?.props ?? {};
    if (!isDestinyMark(props.hab_escolhida) || parseNumber(props.hab_marca_destino_bonus) >= 3)
        return false;
